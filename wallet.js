    ...
    <div class="app">
      <div class="topbar">
        <div>
          ...
          <div class="made-by">Wallet & Tracker</div>
        </div>
        <div style="margin-left:auto;display:flex;align-items:center;gap:.5rem;">
          <div id="firebaseStatus" style="display:none;font-family:monospace;font-size:.52rem;padding:.25rem .6rem;border-radius:8px;background:rgba(22,163,74,.08);"></div>
        </div>
      </div>
      ...
      <div id="wsub-content-wallet" style="display:none;">
        <div class="wallet-big">
          ...
          <div class="wallet-actions">
            <button class="wallet-action-btn" onclick="openDepositModal()">+ Storten</button>
            <button class="wallet-action-btn" onclick="openWithdrawModal()">- Opnemen</button>
      ...
      <!-- JACKS FOTO IMPORT MODAL -->
      <div class="modal-overlay" id="jacksPhotoModal" style="display:none;" onclick="if(event.target===this)closeJacksPhotoImport()">
        <div class="modal-box" style="max-height:88vh;overflow-y:auto;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.7rem;">
            <h3 style="margin:0;">📸 Jacks Import</h3>
            <button onclick="closeJacksPhotoImport()" style="background:none;border:none;font-size:1.2rem;cursor:pointer;color:var(--sub);">✕</button>
          </div>
          <div style="font-family:monospace;font-size:.52rem;color:var(--sub);margin-bottom:.75rem;line-height:1.7;">
            Maak een screenshot van je Jacks betgeschiedenis en upload die hieronder. AI leest de bets automatisch uit.
          </div>
          <label style="display:block;width:100%;padding:.65rem;border-radius:12px;text-align:center;
            ...
            <div style="font-family:monospace;font-size:.48rem;color:var(--sub);margin-top:.2rem;">JPG, PNG of screenshot</div>
          ...
          <div class="modal-actions" style="margin-top:.5rem;">
            <button class="modal-btn cancel" onclick="closeJacksPhotoImport()">Annuleer</button>
            <button class="modal-btn confirm" id="jacksPhotoImportBtn" style="display:none;" onclick="confirmJacksPhotoImport()">📥 Importeren</button>
          </div>
        </div>
      </div>
      ...
      <!-- JACKS IMPORT MODAL -->
      <div class="modal-overlay" id="jacksImportModal" style="display:none;" onclick="if(event.target===this)closeJacksImport()">
        <div class="modal-box" style="max-height:85vh;overflow-y:auto;">
          <div class="modal-header">
            <h3 class="modal-title">🎰 Importeer van Jacks</h3>
            <button class="modal-close-btn" onclick="closeJacksImport()">✕</button>
          </div>
          <div class="modal-body">
            <div style="font-family:monospace;font-size:.52rem;color:var(--sub);margin-bottom:.75rem;line-height:1.7;">
              1. Ga naar <b>jacks.nl → Mijn account → Betgeschiedenis</b><br>
              2. Selecteer alles op de pagina (<b>lang indrukken → Alles selecteren</b>)<br>
              3. Kopieer en plak hieronder
            </div>
            <textarea id="jacksImportText" placeholder="Plak hier je betgeschiedenis van Jacks..."
              style="width:100%;min-height:140px;font-family:monospace;font-size:.55rem;
              padding:.6rem;border-radius:10px;border:1px solid var(--stroke);
              background:var(--card);color:var(--ink);resize:vertical;outline:none;"></textarea>
            <div id="jacksImportPreview" style="margin-top:.6rem;"></div>
            <div class="modal-actions" style="margin-top:.75rem;">
              <button class="modal-btn cancel" onclick="closeJacksImport()">Annuleer</button>
              <button class="modal-btn confirm" onclick="parseJacksImport()">📥 Importeren</button>
            </div>
          </div>
        </div>
      </div>
...
function renderSmartStats() {
  ...
  const srcRows = Object.entries(bySource).map(([src,d]) => {
    ...
    const col = pct>=55?'#16a34a':pct>=40?'#d97706':'#dc2626';
...
function parseJacksImport() {
  ...
  if (preview) {
    preview.innerHTML = `
      <div style="font-family:monospace;font-size:.55rem;color:#15803d;margin-bottom:.4rem;">
...
async function handleJacksPhotoUpload(event) {
  ...
  try {
    ...
    const response = await fetch(`${WORKER}/anthropic`, {
      method: 'POST',
...
Formaat: {"bets":[{"match":"Ajax vs PSV","date":"16-05-2026","pick":"1","odds":2.10,"stake":10.00,"payout":21.00,"status":"gewonnen"}]}
Status opties: "gewonnen", "verloren", "open"
Als je geen bets kunt vinden, geef dan: {"bets":[]}
Datum formaat: dd-mm-yyyy`,
    ...
    if (status) { status.textContent = `✅ ${_jacksParsedBets.length} bet${_jacksParsedBets.length>1?'s':''} herkend`; status.style.color = '#15803d'; }

    if (betsEl) {
      betsEl.innerHTML = _jacksParsedBets.map((b, i) => {
        const statusIcon = b.status === 'gewonnen' ? '✅' : b.status === 'verloren' ? '❌' : '⏳';
        const statusColor = b.status === 'gewonnen' ? '#15803d' : b.status === 'verloren' ? '#dc2626' : '#d97706';
        return `<div style="display:flex;align-items:center;gap:.5rem;padding:.4rem .6rem;
          background:var(--card);border:1px solid var(--stroke);border-radius:10px;margin-bottom:.3rem;">
          <div style="font-size:.9rem;">${statusIcon}</div>
          <div style="flex:1;min-width:0;">
            <div style="font-family:monospace;font-size:.58rem;font-weight:700;color:var(--ink);
              overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">${b.match||'?'}</div>
            <div style="font-family:monospace;font-size:.48rem;color:var(--sub);">
              ${b.pick||'?'} @ ${b.odds||'?'} · €${b.stake||'?'} · ${b.date||'?'}
            </div>
          </div>
          <div style="font-family:monospace;font-size:.55rem;font-weight:800;color:${statusColor};">
            ${b.status === 'gewonnen' ? `+€${((b.payout||0)-(b.stake||0)).toFixed(2)}` :
              b.status === 'verloren' ? `-€${(b.stake||0).toFixed(2)}` : '⏳'}
          </div>
        </div>`;
      }).join('');
    }
