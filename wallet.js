...
function quickBetFromValue(matchId, pick, pickLabel, odds) {
  ...
  // v18.8: zoek matchnaam ook in valueScans/valueBacktest als match niet in state.matches staat
  let matchName = '';
  if (match) {
    matchName = match.home + ' vs ' + match.away;
  } else {
    const scan = (state.valueScans||[]).find(s => String(s.id||s.match?.id) === String(matchId));
    if (scan) matchName = (scan.match?.home||scan.home||'?') + ' vs ' + (scan.match?.away||scan.away||'?');
    if (!matchName) {
      const bt = (state.valueBacktest?.picks||[]).find(p => String(p.fixtureId||p.matchId) === String(matchId));
      if (bt) matchName = bt.matchName || '';
    }
  }
...
function addValueToCombiBuilder(matchId, pick, pickLabel, odds) {
  ...
  // Zoek matchnaam
  const match = (state.matches||[]).find(m => String(m.id) === String(matchId));
  const scan = !match ? (state.valueScans||[]).find(s => String(s.id||s.match?.id) === String(matchId)) : null;
  const home = match?.home || scan?.match?.home || scan?.home || '?';
  const away = match?.away || scan?.match?.away || scan?.away || '?';
