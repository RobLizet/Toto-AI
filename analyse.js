...
function logScanResult(picks) {
  ...
  log.unshift({
    ...
    picks: picks.map(p => ({
      id:         p.id,
      fixtureId:  p.fixtureId || p.id,
      match:      (p.home||'') + ' vs ' + (p.away||''),
      comp:       p.comp || p.compName || '',
      pick:       p.pick || p.betType || '1',
      pickLabel:  p.pickLabel || p.label || '',
      odds:       parseFloat(p.odds||p.homeOdds||2),
      value:      parseFloat(p.value||0),
      confidence: parseInt(p.confidence||5),
      poisson:    p.poisson || null,
      status:     'pending',   // pending | win | lose | void
      score:      null,
      verifiedAt: null
    }))
  });
  // Bewaar max 100 scans
  state.scanLog = log.slice(0, 100);
  saveState();
}

// Auto-verificeer openstaande picks via API
async function verifyScanLog() {
  ...
  for (const { scan, pick } of pending) {
    try {
      const url = \`https://api-football-v1.p.rapidapi.com/v3/fixtures?id=\${pick.fixtureId}\`;
      ...
      // Bepaal uitslag op basis van pick type
      const pickType = (pick.pick||'').toUpperCase();
      if      (pickType === '1')    pick.status = home > away  ? 'win' : 'lose';
      else if (pickType === 'X')    pick.status = home === away ? 'win' : 'lose';
      else if (pickType === '2')    pick.status = away > home  ? 'win' : 'lose';
      else if (pickType === '1X')   pick.status = home >= away ? 'win' : 'lose';
      else if (pickType === 'X2')   pick.status = away >= home ? 'win' : 'lose';
      else if (pickType === 'O25')  pick.status = (home+away) > 2.5 ? 'win' : 'lose';
      else if (pickType === 'U25')  pick.status = (home+away) < 2.5 ? 'win' : 'lose';
      else if (pickType === 'O15')  pick.status = (home+away) > 1.5 ? 'win' : 'lose';
      else if (pickType === 'O35')  pick.status = (home+away) > 3.5 ? 'win' : 'lose';
      else if (pickType === 'BTTS') pick.status = (home > 0 && away > 0) ? 'win' : 'lose';
...
// Render de volledige scan log met statistieken
function renderScanLog() {
  ...
  const avgOdds  = settled.length  ? (settled.reduce((s,p) => s+p.odds,0)/settled.length) : 0;
  ...
  // Value kalibratie (gesegmenteerd)
  const valBuckets = {'0-10':[],'10-20':[],'20-30':[],'30+':[]};
  settled.forEach(p => {
    const v = p.value||0;
    const b = v<10?'0-10':v<20?'10-20':v<30?'20-30':'30+';
    valBuckets[b].push(p.status==='win');
  });
  ...
  el.innerHTML = \`
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;">
      ...
      <div style="display:flex;gap:.4rem;">
        ...
        <button class="small-action-btn danger" onclick="if(confirm('Scan log wissen?')){state.scanLog=[];saveState();renderScanLog();}">🗑</button>
      </div>
    </div>

    <!-- Voortgang balk -->
    <div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:.8rem 1rem;margin-bottom:.8rem;">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:.5rem;margin-bottom:.8rem;">
      \${[
        {val: allPicks.length, lbl:'PICKS', color:'#2563eb'},
        {val: hitrate+'%',     lbl:'HITRATE', color: hitrate>=55?'#16a34a':hitrate>=45?'#d97706':'#dc2626'},
        {val: (roi>=0?'+':'')+roi.toFixed(1)+'%', lbl:'ROI', color: roi>=0?'#16a34a':'#dc2626'},
        {val: avgValue.toFixed(1)+'%', lbl:'AVG VALUE', color:'#7c3aed'}
      ].map(s => \`
        <div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:.6rem .4rem;text-align:center;">
          <div style="font-family:'Bebas Neue',sans-serif;font-size:1.2rem;color:\${s.color};">\${s.val}</div>
          <div style="font-family:'IBM Plex Mono',monospace;font-size:.4rem;color:var(--muted);margin-top:.1rem;">\${s.lbl}</div>
        </div>
      \`).join('')}
    </div>
    ...
    <div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:.8rem 1rem;margin-bottom:.8rem;">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:.4rem;">
        \${Object.entries(valBuckets).map(([range, results]) => {
          const tot = results.length;
          const wr  = tot ? Math.round(results.filter(Boolean).length/tot*100) : null;
          return \`<div style="text-align:center;background:rgba(15,23,42,.04);border-radius:10px;padding:.5rem .3rem;">
            <div style="font-family:'IBM Plex Mono',monospace;font-size:.42rem;color:var(--muted);">\${range}%</div>
            <div style="font-family:'Bebas Neue',sans-serif;font-size:1rem;color:\${wr===null?'var(--muted)':wr>=50?'#16a34a':'#dc2626'};">\${wr===null?'—':wr+'%'}</div>
    ...
    <div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:.8rem 1rem;margin-bottom:.8rem;">
      ...
      \${Object.entries(byType).sort((a,b)=>b[1].total-a[1].total).map(([type,s]) => {
        ...
        return \`<div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.35rem;">
          ...
          <div style="flex:1;background:rgba(15,23,42,.08);border-radius:999px;height:6px;">
            <div style="height:100%;border-radius:999px;background:\${hr>=55?'#16a34a':hr>=45?'#d97706':'#dc2626'};width:\${hr}%;"></div>
          </div>
    ...
    <div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:.8rem 1rem;margin-bottom:.8rem;">
      ...
      \${Object.entries(byComp).sort((a,b)=>b[1].total-a[1].total).slice(0,8).map(([comp,s]) => {
        const hr = Math.round(s.wins/s.total*100);
        return \`<div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.35rem;">
          ...
          <div style="font-family:'IBM Plex Mono',monospace;font-size:.5rem;color:\${hr>=55?'#16a34a':hr>=45?'#d97706':'#dc2626'};font-weight:700;">\${hr}%</div>
    ...
    <!-- Scan lijst -->
    <div style="font-family:'IBM Plex Mono',monospace;font-size:.5rem;font-weight:700;color:var(--text);margin-bottom:.5rem;">
      SCANS (\${log.length})
    </div>
    \${!log.length ? \`
    <div style="text-align:center;padding:2rem;font-family:'IBM Plex Mono',monospace;font-size:.58rem;color:var(--muted);">
      Nog geen scans. Schakel auto scan in via Instellingen.
    </div>\` :
    log.map((scan, si) => {
      ...
      return \`
      <div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:.75rem 1rem;margin-bottom:.5rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.4rem;">
          <div style="font-family:'IBM Plex Mono',monospace;font-size:.52rem;font-weight:700;">#\${log.length-si} · \${scan.date} \${scan.time}</div>
        ...
        \${scan.picks.map(p => \`
        <div style="display:flex;align-items:center;gap:.4rem;padding:.3rem 0;border-top:1px solid var(--border);">
          <div style="width:1.6rem;text-align:center;font-size:.8rem;">
            \${p.status==='win'?'✅':p.status==='lose'?'❌':p.status==='void'?'⬜':'⏳'}
          </div>
          <div style="flex:1;min-width:0;">
            ...
            <div style="font-family:'IBM Plex Mono',monospace;font-size:.44rem;color:var(--muted);">\${p.pickLabel||p.pick} @ \${p.odds} · \${p.value?.toFixed(1)}% value · conf \${p.confidence}/10</div>
          </div>
