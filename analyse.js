    ...
    if (candidates.some(m => m.homeId && m.awayId)) {
      ...
      await Promise.all(candidates.map(async m => {
        ...
        try {
          ...
          // Data kwaliteit score voor schaarse data detectie
          const h2hCount   = h2h?.length    || 0;
          const homeGames  = homeForm?.length || 0;
          const awayGames  = awayForm?.length || 0;
          const dataQuality = h2hCount + homeGames + awayGames; // 0-15 range
          const isSparseData = dataQuality < 5; // weinig data beschikbaar

          // Cap confidence bij schaarse data
...
- H2H patroon telt mee als consistent (3+ vergelijkbare resultaten)
- Motivatie: sluit al bekroonde/gedegradeerde teams worden minder gemotiveerd
...
- Geen uitleg buiten JSON`,
    ...
    const scans = scansRaw.map(s => {
      ...
      if (poisson?.valid) {
        ...
        // Meer data = Poisson betrouwbaarder, maar nooit meer dan 70%
        // Bij weinig data: AI (die seizoenskennis heeft) weegt zwaarder
        const pw = Math.min(0.65,
          0.40                           // basis Poisson weight (verlaagd van 0.45)
          + (poisson.hasXG ? 0.08 : 0)   // xG beschikbaar: +8%
          + (d.h2hCount >= 3 ? 0.04 : 0) // voldoende H2H: +4%
          + (d.homeGames >= 5 ? 0.05 : 0) // voldoende vorm thuis: +5%
          + (d.awayGames >= 5 ? 0.05 : 0) // voldoende vorm uit: +5%
          // Max 65% Poisson — AI vormkennis weegt zwaarder
        );
        const aw = 1 - pw;
      ...
      const makePick = (pick, pickLabel, kans, oddsStr) => {
        ...
        // Gebruik faire value berekening (overround gecorrigeerd)
      ...
      // Confidence: AI en data samen, gecapped bij schaarse data
      ...
      // Value penalty bij schaarse data: hoge value bij weinig data is onbetrouwbaar
      if (isSparse && best.value > 10) {
        best.value = Math.min(best.value, 10); // max 10% value bij schaarse data
      } else if (dataQuality < 8 && best.value > 20) {
        best.value = Math.min(best.value, 20); // max 20% value bij beperkte data
      }
...
KWALITEIT: Noem teams bij naam. Gebruik specifieke cijfers uit context. Kans = jouw schatting na overround-correctie. Confidence op databeschikbaarheid en signaalconsistentie.`,
    ...
    if (tip && tip.pick) {
      ...
      // Toon chat knop na analyse
      const chatBtn = document.getElementById('openChatBtn');
      if (chatBtn) chatBtn.style.display = 'block';

      // Sla analyse context op voor chat
...
function logScanResult(picks) {
  ...
  log.unshift({
    id: Date.now(),
    timestamp: Date.now(), // voor lock tijdsdrempel berekening
    date: today,
...
async function verifyScanLog() {
  ...
  for (const { pick } of pending) {
    try {
      // Via Worker proxy voor betrouwbaarheid
...
// ── Auto-verificatie: check pending picks automatisch ────
// Wordt aangeroepen bij openen Scan Log tab
async function autoVerifyPendingPicks() {
  ...
  log.forEach(scan => {
    scan.picks.forEach(p => {
      if (p.status === 'pending' && p.fixtureId) {
        // Alleen picks van gisteren of eerder automatisch checken
  ...
  for (const [date, items] of Object.entries(byDate)) {
    try {
      // Converteer nl datum (dd-mm-yyyy) naar iso
  ...
  if (verified > 0) {
    saveState();
    // Sync ook naar valueBacktest picks
    syncScanLogToBacktest();
  }
  return verified;
}

// Sync scan log resultaten naar backtest picks voor consistentie
function syncScanLogToBacktest() {
  ...
  log.forEach(scan => {
    scan.picks.forEach(sp => {
      ...
      // Zoek overeenkomend pick in backtest
...
function renderScanLog() {
  ...
  // Auto-verificeer pending picks van gisteren/eerder op de achtergrond
  autoVerifyPendingPicks().then(n => {
    if (n > 0) {
      showToast(`✅ ${n} picks automatisch geverifieerd`);
      renderScanLog(); // herrender met resultaten
    }
  }).catch(() => {});
...
function openMatchChat() {
  ...
  // Reset chat als andere wedstrijd
...
Advies: ${state._lastAnalyseResult.advies || ''}` : '';
    ...
    // Max 6 berichten history meesturen
