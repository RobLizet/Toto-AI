...
- Geen uitleg buiten JSON`,
    ...
    const scans = scansRaw.map(s => {
      ...
      if (poisson?.valid) {
        ...
        const pw = Math.min(0.70,
          0.45                           // basis Poisson weight
          + (poisson.hasXG ? 0.08 : 0)   // xG beschikbaar: +8%
          + (d.h2hCount >= 3 ? 0.05 : 0) // voldoende H2H: +5%
          + (d.homeGames >= 5 ? 0.06 : 0) // voldoende vorm thuis: +6%
          + (d.awayGames >= 5 ? 0.06 : 0) // voldoende vorm uit: +6%
        );
        const aw = 1 - pw;
