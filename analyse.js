...
function renderValueBannerInAnalyse(displayScans, total) {
  ...
  if (!displayScans?.length) {
    const emptyHtml = `<div style="font-family:\'IBM Plex Mono\',monospace;font-size:.58rem;color:rgba(255,255,255,.5);padding:.8rem;text-align:center;">
      Geen value ≥5% gevonden in ${total} wedstrijden.<br>Bookmakers zitten goed in de markt vandaag.
      <button onclick="this.parentElement.parentElement.style.display='none'" style="background:none;border:none;color:rgba(255,255,255,.5);cursor:pointer;margin-left:.5rem;">✕</button>
    </div>`;
  ...
  const html = `
    ...
    ${displayScans.slice(0,6).map(s => {
      ...
      return `<div style="display:flex;align-items:center;padding:.55rem .9rem;border-bottom:1px solid rgba(255,255,255,0.09);cursor:pointer;" onclick="openValueAnalysis('${s.match.id}')">
        <div style="flex:1;">
          ...
          <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.5rem;color:rgba(255,255,255,.5);">${s.pickLabel} · ${s.kans}%${s.poissonUsed?(s._hasXG?' (P+AI+xG)':' (P+AI)'):s._hasXG?' (xG)':''} · ${s.reason}</div>
          <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.46rem;color:rgba(255,255,255,.5);">📊 ${s.bookmaker||''} · ½K ${(s.kelly||0).toFixed(1)}% · 🎲 ${s.confidence||'?'}/10</div>
        </div>
    ...
    <div style="padding:.4rem .9rem;text-align:right;">
      <button onclick="this.parentElement.parentElement.style.display='none'" style="background:none;border:none;color:rgba(255,255,255,.5);cursor:pointer;font-size:.9rem;">✕</button>
    </div>
  `;
  ...
  const renderPick = (s, geldig) => {
    ...
    return `<div style="display:flex;align-items:center;padding:.5rem .9rem;
      ...
      <div style="flex:1;">
        ...
        <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.46rem;color:rgba(255,255,255,.5);">
    ...
    <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(0,190,196,.25);border-radius:14px;
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;
        ...
        <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.6rem;font-weight:800;color:#00BEC4;">
          ⚡ SCAN RESULTATEN · ${teltMee.length} picks <span style="color:rgba(255,255,255,.5);font-weight:400;">van ${scans.length}</span>
        </div>
        <button onclick="document.getElementById('analyseScanResults').innerHTML=''"
          style="background:none;border:none;color:rgba(255,255,255,.5);cursor:pointer;font-size:.85rem;">✕</button>
      </div>
      ${teltMee.map(s => renderPick(s, true)).join('')}
      ${teltNiet.length ? `
        <div style="padding:.35rem .9rem;font-family:\'IBM Plex Mono\',monospace;font-size:.48rem;
          color:rgba(255,255,255,.5);background:var(--card-bg,rgba(0,0,0,.03));border-top:1px solid rgba(255,255,255,0.09);">
...
function openCardPopup(type, data) {
  ...
  if (type === 'scan') {
    ...
    content = `
      ...
      <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.5rem;color:rgba(255,255,255,.5);margin-bottom:.85rem;">
        ${s.comp||''} · ${s.date||''}
      </div>
    ...
    content = `
      ...
      <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.5rem;color:rgba(255,255,255,.5);margin-bottom:.85rem;">${b.date||''} · ${b.markt||'1X2'}</div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.5rem;margin-bottom:.85rem;">
    ...
    content = `
      <div style="display:flex;align-items:center;gap:.75rem;margin-bottom:.85rem;">
        ...
        <div style="flex:1;text-align:center;">
          ...
          <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.5rem;color:rgba(255,255,255,.5);">${m.comp||''} · ${m.date||''} ${m.time||''}</div>
        </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.5rem;margin-bottom:.85rem;">
        <div style="background:rgba(0,190,196,.08);border:1px solid rgba(0,190,196,.2);border-radius:12px;padding:.65rem;text-align:center;">
          <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.44rem;color:rgba(255,255,255,.5);font-weight:700;">1 THUIS</div>
        ...
        <div style="background:rgba(180,83,9,.08);border:1px solid rgba(180,83,9,.2);border-radius:12px;padding:.65rem;text-align:center;">
          <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.44rem;color:rgba(255,255,255,.5);font-weight:700;">X GELIJK</div>
        ...
        <div style="background:rgba(220,38,38,.08);border:1px solid rgba(220,38,38,.2);border-radius:12px;padding:.65rem;text-align:center;">
          <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.44rem;color:rgba(255,255,255,.5);font-weight:700;">2 UIT</div>
          <div style="font-family:\'Bebas Neue\',sans-serif;font-size:1.4rem;color:#dc2626;">${m.awayOdds}</div>
        </div>
      </div>` : `<div style="font-family:\'IBM Plex Mono\',monospace;font-size:.54rem;color:rgba(255,255,255,.5);text-align:center;padding:.75rem;margin-bottom:.75rem;">Geen odds beschikbaar</div>`}
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.5rem;">
      ...
      document.getElementById('rb-tip').innerHTML = `
        <div style="background:linear-gradient(135deg,rgba(0,190,196,.06),rgba(0,190,196,.06));
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;background:${tv>=5?'rgba(0,190,196,.08)':'rgba(100,116,139,.05)'};
            ...
            <div>
              <div style="font-family:monospace;font-size:.52rem;color:rgba(255,255,255,.5);font-weight:700;">⚡ VALUE</div>
              <div style="font-family:monospace;font-size:.5rem;color:rgba(255,255,255,.5);">${tip.kans}% × ${tip.odds}</div>
            </div>
  ...
  if (!allMatches.length && !(state.valueScans||[]).length) {
    ...
    if (card) {
      card.innerHTML = `<div style="text-align:center;padding:1.5rem;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.09);border-radius:14px;">
        ...
        <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.54rem;color:rgba(255,255,255,.5);margin-bottom:.8rem;line-height:1.6;">Laad eerst wedstrijden via het Wedstrijden tabblad om combi tips te genereren.</div>
...
function renderTop3EnCombi(result) {
    ...
    return `<div onclick="openTipPopup(${i})" style="background:var(--card-bg-dark,rgba(0,0,0,.06));border:1px solid rgba(0,0,0,.1);border-radius:12px;padding:.75rem .9rem;margin-bottom:.5rem;cursor:pointer;active:opacity:.85;">
      ...
      <div style="margin-bottom:.45rem;">
        <div style="display:flex;justify-content:space-between;margin-bottom:.2rem;">
          <span style="font-family:monospace;font-size:.48rem;color:rgba(255,255,255,.5);font-weight:700;">ZEKERHEID</span>
  ...
  card.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.7rem;">
      ...
      <div style="font-family:monospace;font-size:.5rem;color:rgba(255,255,255,.5);">📅 ${new Date().toLocaleString('nl-NL',{weekday:'short',day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'})}</div>
    </div>
    ${top3Html}
...
function renderScanLog() {
  ...
  if (_milestone >= 100) {
    ...
    html += '<div style="background:linear-gradient(135deg,rgba(0,190,196,.08),rgba(0,190,196,.06));border:1.5px solid rgba(0,190,196,.2);border-radius:16px;padding:.9rem 1rem;margin-bottom:.8rem;">'
      ...
      // Samenvatting
      + '<div style="background:var(--card-bg,rgba(0,0,0,.04));border-radius:12px;padding:.6rem .7rem;margin-bottom:.65rem;font-family:\'IBM Plex Mono\',monospace;font-size:.60rem;line-height:1.8;color:rgba(255,255,255,.5);">'
      ...
      + Object.entries(byOdds).map(([range, results]) => {
          ...
          return '<div style="text-align:center;background:var(--card-bg,rgba(0,0,0,.04));border-radius:10px;padding:.4rem .2rem;">'
            + '<div style="font-family:\'IBM Plex Mono\',monospace;font-size:.38rem;color:rgba(255,255,255,.5);">' + range + '</div>'
            + '<div style="font-family:\'Bebas Neue\',sans-serif;font-size:.9rem;color:' + col + ';">' + (wr===null?'—':wr+'%') + '</div>'
            + '<div style="font-family:\'IBM Plex Mono\',monospace;font-size:.36rem;color:rgba(255,255,255,.5);">' + tot + 'x</div>'
            + '</div>';
        }).join('')
      + '</div></div>'
      ...
      + Object.entries(byConf).map(([conf, results]) => {
          ...
          return '<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;">'
            ...
            + '<div style="font-family:\'IBM Plex Mono\',monospace;font-size:.38rem;color:rgba(255,255,255,.5);">conf ' + conf + '</div>'
            + '</div>';
        }).join('')
      + '</div></div>'
...
function showScanLogStatsPopup() {
  ...
  if (Object.keys(byType).length) {
    body += `<div style="background:var(--card-bg,rgba(0,0,0,.03));border-radius:12px;padding:.7rem .85rem;margin-bottom:.6rem;">
      <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.5rem;font-weight:700;color:rgba(255,255,255,.5);margin-bottom:.5rem;">🎯 PER PICK TYPE</div>`;
    Object.entries(byType).sort((a,b)=>b[1].total-a[1].total).forEach(([type,s]) => {
      ...
      body += `<div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.3rem;">
        ...
        <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.48rem;color:rgba(255,255,255,.5);min-width:4rem;text-align:right;">${hr}% (${s.total})</div>
      </div>`;
    });
    body += '</div>';
  }
  ...
  if (Object.keys(byComp).length) {
    body += `<div style="background:var(--card-bg,rgba(0,0,0,.03));border-radius:12px;padding:.7rem .85rem;margin-bottom:.6rem;">
      <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.5rem;font-weight:700;color:rgba(255,255,255,.5);margin-bottom:.5rem;">🏆 PER COMPETITIE</div>`;
    Object.entries(byComp).sort((a,b)=>(b[1].roi/b[1].total)-(a[1].roi/a[1].total)).slice(0,8).forEach(([comp,s]) => {
      ...
      body += `<div style="display:flex;align-items:center;gap:.4rem;margin-bottom:.3rem;">
        ...
        <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.42rem;color:rgba(255,255,255,.5);">${s.total}x</div>
      </div>`;
    });
    body += '</div>';
  }
  ...
  // Per odds range
  body += `<div style="background:var(--card-bg,rgba(0,0,0,.03));border-radius:12px;padding:.7rem .85rem;margin-bottom:.6rem;">
    <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.5rem;font-weight:700;color:rgba(255,255,255,.5);margin-bottom:.5rem;">📊 PER ODDS RANGE</div>
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:.3rem;">`;
  Object.entries(byOdds).forEach(([range,results]) => {
    ...
    body += `<div style="text-align:center;background:var(--card-bg,rgba(0,0,0,.04));border-radius:8px;padding:.4rem .2rem;">
      <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.4rem;color:rgba(255,255,255,.5);">${range}</div>
      <div style="font-family:\'Bebas Neue\',sans-serif;font-size:.9rem;color:${wr===null?'var(--sub)':hrColor(wr)};">${wr===null?'—':wr+'%'}</div>
      <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.38rem;color:rgba(255,255,255,.5);">${tot}x</div>
    </div>`;
  });
  body += '</div></div>';
  ...
  // Value kalibratie
  body += `<div style="background:var(--card-bg,rgba(0,0,0,.03));border-radius:12px;padding:.7rem .85rem;margin-bottom:.6rem;">
    <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.5rem;font-weight:700;color:rgba(255,255,255,.5);margin-bottom:.5rem;">📐 VALUE KALIBRATIE</div>
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:.3rem;">`;
  Object.entries(vb).forEach(([range,results]) => {
    const tot = results.length;
    const wr  = tot ? Math.round(results.filter(Boolean).length/tot*100) : null;
    body += `<div style="text-align:center;background:var(--card-bg,rgba(0,0,0,.04));border-radius:8px;padding:.4rem .2rem;">
      <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.42rem;color:rgba(255,255,255,.5);">${range}</div>
      <div style="font-family:\'Bebas Neue\',sans-serif;font-size:1rem;color:${wr===null?'var(--sub)':hrColor(wr)};">${wr===null?'—':wr+'%'}</div>
      <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.4rem;color:rgba(255,255,255,.5);">${tot} picks</div>
    </div>`;
  });
  body += '</div></div>';
...
function showManualVerify(scanId, pickId, pick, matchName) {
  ...
  modal.innerHTML = `
    <div style="background:rgba(255,255,255,0.05);border-radius:20px 20px 0 0;padding:1.25rem 1.25rem 2rem;
      ...
      <div style="font-family:\'IBM Plex Mono\',monospace;font-size:.56rem;color:rgba(255,255,255,.5);margin-bottom:1rem;line-height:1.6;">
        ${matchName}<br>
        Pick: <b style="color:#ffffff;">${pick}</b>
      </div>
      <div style="margin-bottom:1rem;">
        <label style="font-family:\'IBM Plex Mono\',monospace;font-size:.52rem;color:rgba(255,255,255,.5);display:block;margin-bottom:.4rem;">
          EINDSTAND (bijv. 2-1)
        </label>
      ...
      <div style="display:flex;gap:.5rem;">
        <button onclick="document.getElementById('manual-verify-modal').remove()"
          ...
          font-size:.6rem;font-weight:700;color:rgba(255,255,255,.5);cursor:pointer;">
          Annuleren
        </button>
