...
function renderMatchCard(m) {
  ...
  card.innerHTML = `
    ...
    <div class="match-teams">
      ...
      <div class="vs-col">
        ${m.score ? `<div class="score-txt">${m.score}</div>` : '<div class="vs-txt">VS</div>'}
      </div>
    ...
    <!-- Odds buttons -->
    ${m.homeOdds !== '—' ? `
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.35rem;padding:.45rem .9rem .6rem;">
      <button class="odds-btn" onclick="event.stopPropagation();openBetModal(event,'${m.id}','1','Thuis',${m.homeOdds})">
        ...
        <span class="odds-val">${m.homeOdds}</span>
      </button>
    ...
    <div style="padding:.5rem .9rem .65rem;">
      <button onclick="event.stopPropagation();selectMatchAndAnalyse('${m.id}')"
        style="width:100%;padding:.45rem;border-radius:9px;background:rgba(219,39,119,.08);
...
async function runMultiScan() {
  ...
  for (let i = 0; i < favs.length; i++) {
    ...
    try {
      ...
      let loaded = await Promise.race([loadFromAPIFootball(comp, null), loadTimeout]);
      if (!loaded && fdKey && fdCode) {
        loaded = await Promise.race([loadFromFD(fdCode, fdKey, comp), loadTimeout]);
      }
    } catch(e) { console.warn('[MultiScan] laden mislukt:', comp, e.message); }
    ...
    const leagueId = COMP_IDS[comp];
    try {
      if (leagueId) await Promise.race([
        fetchOddsForMatches(leagueId, null),
