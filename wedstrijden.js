  ...
  screen.innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:.35rem;" id="compGrid">
        ${COMP_LIST.map(c => {
          ...
          return `<div class="comp-chip${isActive?' active':''}${isFav?' fav':''}" id="comp-${c.key}"
            ...
            cursor:pointer;background:${isActive?'rgba(124,58,237,.12)':'rgba(255,255,255,.7)'};
            border:1.5px solid ${isActive?'rgba(124,58,237,.4)':'rgba(15,23,42,.08)'};"
            ...
            <div style="font-family:'IBM Plex Mono',monospace;font-size:.38rem;font-weight:700;
              color:${isActive?'#7c3aed':'var(--sub)'};overflow:hidden;white-space:nowrap;
    ...
    <div style="display:flex;gap:.4rem;margin-bottom:.6rem;flex-wrap:wrap;">
      ...
      <button onclick="openCompDetail(state.activeComp)"
        style="font-family:'IBM Plex Mono',monospace;font-size:.52rem;font-weight:700;
        padding:.35rem .8rem;border-radius:999px;cursor:pointer;
        background:rgba(124,58,237,.1);border:1px solid rgba(124,58,237,.3);color:#7c3aed;">
        📊 STAND & INFO
      </button>
    </div>
  ...
  const oddsCards = hasOdds ? `
    ...
    <div style="padding:.4rem .9rem .5rem;">
      <div style="font-family:'IBM Plex Mono',monospace;font-size:.5rem;color:var(--sub);
        ...
        background:rgba(15,23,42,.03);border-radius:8px;border:1px dashed rgba(15,23,42,.1);">
...
async function openCompDetail(compKey) {
  ...
  [['stand','Stand'],['scorers','Topscorers'],['wedstrijden','Wedstrijden']].forEach(([t,label]) => {
    ...
    btn.id = 'ctab-' + t;
    btn.style.cssText = 'font-family:IBM Plex Mono,monospace;font-size:.55rem;font-weight:700;padding:.5rem .75rem;border:none;background:none;cursor:pointer;border-bottom:2px solid ' + (t==='stand'?'#be185d':'transparent') + ';color:' + (t==='stand'?'#be185d':'var(--sub)') + ';';
