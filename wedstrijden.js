...
function renderWedstrijdenScreen() {
  ...
  screen.innerHTML = `
    ...
    <!-- Scan resultaten panel (nu in Analyse tab) -->
...
async function fetchOddsForMatches(leagueId, _apiKey) {
  ...
  if (!oddsData) {
    ...
    // Haal datum op uit geladen matches voor deze league
    ...
    for (const bm of bookmakers) {
      try {
        // Probeer eerst met datum (meest precies)
        ...
        // Fallback: next=10 (werkt ook voor toekomstige wedstrijden)
...
async function runMultiScan() {
  ...
  // Sla op in state.valueScans voor Analyse tab
...
function renderScanResults(scans, restored = false) {
  ...
  const normalized = scans.map(s => {
    ...
    // Probeer teamnamen uit state.matches te halen als ze missen
...
function openJacks(matchId) {
  ...
  const compSlugMap = {
    ...
    'Süper Lig':             'super-lig',
    'Friendlies':            'internationaal',
    'FIFA World Cup':        'wk-2026',
  };
