...
async function loginWithEmail() {
  ...
  const errEl    = document.getElementById('loginError');
  if (!email||!password) { if(errEl) errEl.textContent='Vul email en wachtwoord in'; return; }
  try {
...
async function registerWithEmail() {
  ...
  const errEl    = document.getElementById('loginError');
  if (!email||!password) { if(errEl) errEl.textContent='Vul alle velden in'; return; }
  if (password.length<6) { if(errEl) errEl.textContent='Wachtwoord min. 6 tekens'; return; }
  try {
    if (!_firebaseAuth) throw new Error('Firebase niet beschikbaar');
...
async function loginWithGoogle() {
  ...
  try {
    ...
    if (e.code !== 'auth/popup-closed-by-user') {
      if (errEl) errEl.textContent = 'Google login mislukt: ' + e.message;
    }
  }
}
