    ...
    await Promise.all(knownLeagueIds.map(async leagueId => {
      try {
        ...
        (d.response || []).forEach(item => {
          ...
          if (!fix) return;
          if (['FT','AET','PEN'].includes(fix.status?.short || '')) return;
        ...
        try {
          ...
          (d.response || []).forEach(item => {
            ...
            if (!seen.has(String(fix.id))) {
              ...
              let hPct = 33, dPct = 33, aPct = 34;
              if (homeOdds !== '—' && drawOdds !== '—' && awayOdds !== '—') {
                const inv2 = 1/parseFloat(homeOdds) + 1/parseFloat(drawOdds) + 1/parseFloat(awayOdds);
                hPct = Math.round((1/parseFloat(homeOdds))/inv2*100);
                dPct = Math.round((1/parseFloat(drawOdds))/inv2*100);
                aPct = 100 - hPct - dPct;
              }
              matches.push({
                id: String(fix.id), source: 'apif',
                ...
                awayLogo: fix.teams?.away?.logo || '',
                homeOdds, drawOdds, awayOdds,
                homePct: hPct, drawPct: dPct, awayPct: aPct,
