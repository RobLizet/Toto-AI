    ...
    if (path === '/health') {
      try {
        ...
        try {
          ...
          clvGaps = gaps.length;
          if (gaps.length) warnings.push(`settled_zonder_clv(${gaps.length}: ${gaps.slice(0, 3).map(g => g.id).join(', ')}${gaps.length > 3 ? ', ...' : ''})`);
