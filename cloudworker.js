...
// maar geen enkel duel had een xG-cijfer (de bookmaker/API-Football-dekkingsgat uit de 18-08-backtest).
...
async function fetchTeamXgAvg(teamId, season, env, seenTeams) {
  ...
  try {
    ...
    if (!ftFixtures.length) return { avg: null, n: 0, checked: 0 };
    const xgs = [];
    for (const f of ftFixtures) {
      ...
      const xgStat = teamStat?.statistics?.find(s => s?.type === 'expected_goals');
      const val = xgStat ? parseFloat(xgStat.value) : null;
      if (Number.isFinite(val)) xgs.push(val);
    }
    return { avg: xgs.length ? xgs.reduce((a, b) => a + b, 0) / xgs.length : null, n: xgs.length, checked: ftFixtures.length };
