...
const VERSION = 'v255'; // v255: `finishedFx is not defined` in settleModelTips opgeruimd. v202 zette `const finishedFx = []` in settleShadowPicks en de lezer `for (const fx of finishedFx)` in settleModelTips -- twee functies, dus een ReferenceError bij ELKE settle-run sinds v202 (git log -S wijst beide regels aan diezelfde commit toe; de commit-tekst 'Team-ID-koppeling via fixture-object in settleModelTips' verraadt de bedoeling). Die lus heeft dus nooit gedraaid -- wat verklaart waarom v209 constateerde dat de Elo leeg bleef. Impact was beperkt en de foutmelding loog erover: de throw kwam NA de PATCHes en NA '[AI-tips] N model-tips afgerekend', dus de afrekening was al klaar; alleen de Elo-lus viel weg. Geschrapt i.p.v. gerepareerd: sinds v239 doet sweepEloFromResults dit op de cron via dezelfde updateEloForFixture (r2495, skipDup) -- die functie blijft in gebruik, alleen deze dode dubbele aanroep gaat eruit. Ook de dode array in settleShadowPicks weg (werd gevuld, nooit gelezen). GEEN gedragswijziging behalve dat de valse error verdwijnt. STAND VAN 16-07 -- vier fouten gevonden op EEN cron-run van 13:00 UTC, geen ervan geraden: (1) hour was NL-tijd, 2 fullScans/dag vielen stil af [v254]; (2) mmc.pick was varchar(1), nul goal-markt-rijen sinds 11-06 [v254 + migratie naar varchar(12)]; (3) apif's melder hing aan de dode uitgang [v254]; (4) deze. NOG OPEN: de 2-van-2 date-call-weigering. GEMETEN, niet vermoed: op de rustigste dag van het jaar, met verwaarloosbaar dagverbruik, werden beide parallelle date-calls geweigerd NA 3 retries met backoff. Twee calls is geen burst -- dat wijst op de gedeelde Cloudflare-egress, niet op quota. Vandaag redde het v130-vangnet (missingNext via /fixtures?league=&next=10) het nog, maar NEXT_LEAGUES heeft nul overlap met de 19 FASE 2-competities: vanaf 20-07 levert zo'n weigering nul wedstrijden, nul picks, en dat ziet er precies uit als een rustige dag. Kandidaten (serialiseren / andere egress / host-volgorde) pas kiezen als de v254-log zegt WELKE host weigert en wat de fallback teruggaf. Openstaand vermoeden dat de log moet beslechten: beide hosts krijgen dezelfde env.FOOTBALL_KEY (x-apisports-key EN x-rapidapi-key) terwijl dat verschillende sleutelsoorten zijn -- als de fallback daardoor structureel leeg antwoordt, is 'twee hosts' in werkelijkheid een host. Dezelfde constructie staat in de proxy (r1226), bewust onaangeroerd tot dit vaststaat.
