...
export default {
  async fetch(request, env) {
    ...
    if (path === '/vvv-news') {
      try {
        const rss = 'https://news.google.com/rss/search?q=%22VVV-Venlo%22&hl=nl&gl=NL&ceid=NL:nl';
        const r = await fetch(rss, {
          headers: { 'User-Agent': 'Mozilla/5.0 (ProMatchXI news proxy)' },
          cf: { cacheTtl: 900, cacheEverything: true },
        });
        if (!r.ok) return json({ items: [], error: 'bron onbereikbaar' });
        ...
        for (const b of blocks.slice(0, 25)) {
          ...
          let title = grab(body, 'title');
          const link = grab(body, 'link');
          const pubDate = grab(body, 'pubDate');
          const source = grab(body, 'source');
          // Google News zet vaak " - Bron" achter de titel; strip dat als de bron al apart staat.
