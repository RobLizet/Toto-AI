...
async function fetchOddsForFixtures(fixtureIds, env, maxCalls = 36, enableGoals = false, matches = null) {
  const oddsMap = {};
  ...
  try {
      ...
      for (const key of plan.keys()) {
        ...
        try {
          for (let page = 1; page <= ODDS_MAX_PAGES; page++) {
            ...
            if (r && r.rateLimited) { rlHits++; comboFouten++; break; }
            if (!r?.length) break;
            r.forEach(item => {
      ...
      for (const date of [today, tomorrow]) {
        ...
        try {
          for (let page = 1; page <= ODDS_MAX_PAGES; page++) {
            ...
            oddsCallsUsed++;
            if (r && r.rateLimited) { bulkRlHits++; break; }
            if (!r?.length) break;
            r.forEach(item => {
              const fid = item.fixture?.id;
              if (fid && fixtureSet.has(fid) && !oddsMap[fid]) parseConsensus([item], fid);
    ...
    if (missing.length && oddsCallsUsed < maxCalls) {
      ...
      rs.forEach((r, j) => {
        ...
        if (r.value && r.value.rateLimited) { fbRl++; return; }
    ...
    if (enableGoals) {
      ...
      rs.forEach((r, j) => {
        if (r.status === 'fulfilled') {
          if (r.value && r.value.rateLimited) { rl++; return; }
...
async function runScanTest(env, leagueIds = [1, 113, 103], enableGoals = false, maxCap = null) { // 1=WK 2026, 113=Allsvenskan, 103=Eliteserien
  ...
  const oddsMap = await fetchOddsForFixtures(fixtureIds, env, 60, enableGoals, batch); // v243: per competitie
