      ...
      try {
        ...
        for (const fid in byFix) {
          ...
          for (const r of three) {
            if (r.pick === 'X') continue;                        // v201: app wed geen gelijkspel als value-tip
            const implied = ((1 / Number(r.market_consensus_odds)) / invSum) * 100; // de-vig
            if (implied < 15) continue;                          // v201: longshot-guard — extreme underdog is geen value-tip
