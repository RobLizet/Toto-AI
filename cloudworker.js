...
async function handleAnthropic(request, env) {
  ...
  const MAX_USER_CALLS_PER_DAY  = 5;   // per gebruiker per dag (pre-launch; verhogen bij testfase)
  const MAX_GLOBAL_CALLS_PER_DAY = 60; // totaal per dag over alle gedeelde-key-gebruikers
  ...
  async fetch(request, env) {
    ...
    if (path === '/health') {
      try {
        ...
        // v177: alleen waarschuwen tijdens werkelijke scan-uren (cron: 06 + 12-16 UTC).
        // Buiten die uren zijn er geen scans gepland → geen valse "scan_verouderd" 's avonds/'s nachts.
        // Een gemiste 16:00-scan wordt nog tijdens uur 16 gevangen (>75 min sinds 15:00).
        const activeHours = (hour === 6) || (hour >= 12 && hour <= 16);
