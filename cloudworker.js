...
async function settleShadowPicks(env) {
  try {
    ...
    // v169: één gebatchte fixtures-call (?ids=a-b-c) i.p.v. 15 losse — bespaart subrequests
    const data = await apif(`/fixtures?ids=${fixtureIds.join('-')}`, env);
    const resultMap = {};
    (data || []).forEach(fx => {
      const st = fx.fixture?.status?.short;
      if (!['FT','AET','PEN'].includes(st)) return;
      resultMap[String(fx.fixture?.id)] = { home: fx.goals?.home ?? 0, away: fx.goals?.away ?? 0 };
    });
    let settled = 0;
    for (const r of rows) {
