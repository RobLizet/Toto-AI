...
async function handleAPIFootball(path, env, bypassCache = false) {
  ...
  for (const host of hosts) {
    try {
      ...
      if (cacheable && data && !data.errors && _hasPayload(data.response)) {
        try {
        ...
        _cacheDbg = data?.errors ? 'skip:errors' : (!_hasPayload(data?.response) ? 'skip:empty-payload' : 'skip:onbekend');
      }
