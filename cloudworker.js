  ...
  try {
    ...
    if (_perLeague) {
      ...
      for (const key of plan.keys()) {
        ...
        } catch(e) { console.error(`[Odds] ${date} league ${lg} fout:`, e.message); }
      }
      console.log(`[Odds] per-competitie klaar: ${fixtureIds.filter(id => oddsMap[id]).length}/${fixtureIds.length} gedekt na ${oddsCallsUsed} calls over ${plan.size} combinaties`);
    } else {
