...
export default {
  async fetch(request, env) {
    ...
    if (path === '/push-devices') {
      ...
      let d;
      try {
        const r = await fetch(`https://onesignal.com/api/v1/players?app_id=${appId}&limit=50`,
          { headers: { 'Authorization': `Basic ${apiKey}` } });
        d = await r.json();
        if (!r.ok) return json({ ok: false, gemeten: false, http: r.status, antwoord: d, version: VERSION }, 502);
      } catch (e) {
        // Een mislukte call is GEEN 'geen toestellen'.
        return json({ ok: false, gemeten: false, fout: e.message, version: VERSION }, 502);
      }
      const spelers = Array.isArray(d?.players) ? d.players : null;
      if (spelers === null) {
        return json({ ok: false, gemeten: false, reden: 'geen players-array in het antwoord', antwoord: d, version: VERSION }, 502);
      }
      // typeof-checks i.p.v. `||`: 0 sessies en notification_types 0 zijn MEETWAARDEN,
      // geen ontbrekende data.
      const toestellen = spelers.map(p => ({
        id: p.id ?? null,
      ...
      return json({
        ok: true, gemeten: true,
        totaal: (typeof d?.total_count === 'number') ? d.total_count : null,
        getoond: toestellen.length,
        abonnees: toestellen.filter(t => t.abonnee === true).length,
        met_admin_tag: toestellen.filter(t => t.admin_tag === true).length,
        admin_en_abonnee: toestellen.filter(t => t.admin_tag === true && t.abonnee === true).length,
