...
async function runScan(env, force = false, skipTellerReset = false) {
  ...
  if (!allMatches.length) {
    ...
    const scanData0 = { lastRun: new Date().toISOString(), lastMatchCount: 0,
      ...
      oddsAlleMet: 0, oddsAlleVan: 0 }; // v295: gemeten nul (geen fixtures in venster), niet 'onbekend'
  ...
  async fetch(request, env) {
    ...
    if (path === '/health') {
      try {
        ...
        try {
          const sc = await sb(env, 'scan_status', 'GET', null, '?id=eq.current&select=last_match_count,last_with_odds,last_without_odds,last_odds_alle_met,last_odds_alle_van&limit=1');
          const r0 = sc?.[0];
          if (r0) {
