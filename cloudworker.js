      ...
      for (const key of plan.keys()) {
        ...
        try {
          for (let page = 1; page <= ODDS_MAX_PAGES; page++) {
            ...
            const _cb = Date.now();
            const r = await apif(`/odds?date=${date}&league=${lg}&season=${seizoen}&bet=1&page=${page}&_cb=${_cb}`, env);
            oddsCallsUsed++;
      ...
      for (const date of [today, tomorrow]) {
        ...
        try {
          for (let page = 1; page <= ODDS_MAX_PAGES; page++) {
            if (oddsCallsUsed >= maxCalls) break;
            const _cb = Date.now();
            const r = await apif(`/odds?date=${date}&bet=1&page=${page}&_cb=${_cb}`, env);
            oddsCallsUsed++;
