...
async function saveOddsSnapshots(oddsMap, matches, env, goalStatus = null) {
  ...
  matches.forEach(m => {
    ...
    // v293: alleen schrijven als er echt een gemeten reden is. typeof-check i.p.v. `||`
    // zodat een lege string nooit stilzwijgend als 'niet gemeten' wegvalt.
    if (_gs && typeof _gs[m.fixtureId] === 'string' && _gs[m.fixtureId]) row.goal_odds_status = _gs[m.fixtureId];
