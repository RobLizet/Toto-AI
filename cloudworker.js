...
export default {
  async fetch(request, env) {
    ...
    if (path === '/scan-now') {
      try {
        const st = await sb(env, 'scan_status', 'GET', null, '?id=eq.current&select=last_run&limit=1');
        const last = st?.[0]?.last_run ? new Date(st[0].last_run).getTime() : 0;
