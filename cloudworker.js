...
const VERSION = 'v254'; // v254: DRIE FOUTEN, ALLE DRIE OP DE CRON-RUN VAN 13:00 UTC GEVONDEN, GEEN ERVAN GERADEN. (1) `hour` IN runScan WAS NEDERLANDSE TIJD MAAR HEETTE UTC. De cron-handler rekent in echte UTC (fullScan = hour===6 || hour>=12 && hour<=22) en roept runScan aan; runScan deed getUTCHours()+2 en toetste dat tegen scanFrom=6/scanTo=23, een venster dat overal 'UTC' heet. Twee functies, twee betekenissen van dezelfde naam. GEMETEN: de run met scheduledTime 1784206846000 = 13:00:46 UTC logde 'Start scan (15:00 UTC)'. Gevolg: 21:00 UTC gaf hour=23 -> 23>=23 -> skip; 22:00 UTC gaf hour=24 -- een uur dat niet bestaat, want er zat geen modulo op -> skip. Twee van de twaalf geplande fullScans vielen elke dag stil af NADAT de cron ze had ingepland: 23:00 en 00:00 NL, en snapshotOddsOnly begint pas om 23:00 UTC, dus een gat van twee uur vlak voor de slotkoers. RAAKT DE GESCHIEDENIS: v238 schreef 'de scans van 20/21/22 UTC vonden geen wedstrijden' toe aan MAX_SCANS_PER_DAY; die van 21 en 22 UTC bereikten de fixture-fetch nooit. Nu getUTCHours(): alle 12 draaien, zoals v188 bedoelde. Kost ~160 odds-calls/dag extra op 7500 en 2 AI-batches (~$0,007). (2) apif's DIAGNOSELOG HING AAN DE DODE UITGANG -- mijn eigen v252. De vlag ging naar `return out`, de console.warn bleef op de `leeg`-constructie die alleen bereikbaar is als GEEN van beide hosts returnt. GEMETEN: de scan van 13:00 UTC meldde 'FIXTURES GEWEIGERD: 2 van 2' terwijl een zoekopdracht op 'rate-limited' in de Cloudflare-logs NUL treffers gaf. Dat sluit routes uit: geen exception en geen errors.key/token (beide zouden naar `leeg` vallen) -> host 2 antwoordde met een LEGE array en het liep langs de stille uitgang. De meter werkte, de melder niet, en daardoor is niet te zien WELKE host weigert of WAAROM de fallback niets levert. Nu: hosts hebben een naam, _rlHost legt de weigeraar vast op het moment zelf (niet afleiden -- met twee hosts is 'dus host 1' toevallig waar en dat is precies de aanname die hier misgaat), en beide uitgangen loggen host + http-status + errors + results + message. Puur diagnostisch: geen drempel, formule of selectie gewijzigd. OPEN: beide hosts krijgen DEZELFDE env.FOOTBALL_KEY mee (x-apisports-key en x-rapidapi-key), en dat zijn verschillende sleutelsoorten -- als de fallback daardoor structureel leeg is, is 'twee hosts' in werkelijkheid een host. Hypothese, niet gemeten; de eerstvolgende weigering wijst het uit. Dezelfde constructie staat ook in de proxy (r1226), bewust niet aangeraakt tot dit vaststaat. (3) DE VENSTER-LOGREGEL LOOG: hardgecodeerd '06:00-22:00 UTC (gelijk aan cron)' terwijl scanTo=23 en de cron 0 6 + 0 12-22 + 0 23 + 0 0-5 doet. Leest nu zijn eigen variabelen. NIET IN DEZE PUSH: de 2-van-2-weigering zelf (architectuur: serialiseren / andere egress / host-volgorde -- eerst de meting uit (2) afwachten, want 3 retries met backoff faalden al en twee parallelle calls zijn geen burst) en `finishedFx is not defined` in settleModelTips (v202, crasht na de PATCHes en na de logregel, dus alleen de Elo-lus valt weg en sweepEloFromResults dekt dat sinds v239). // v253: MELDING BIJ NIEUW VVV-VENLO-NIEUWS (ADMIN-ONLY). /vvv-news was sinds v231 puur pull: de tab haalde de twee bronnen op zodra je hem opende en er werd NIETS bewaard, dus het begrip 'nieuw sinds vorige keer' bestond niet en er was per definitie niets dat een push kon afvuren. Nu: tabel vvv_news_seen (link als PK -- de enige stabiele sleutel, want pub_date ontbreekt in de clubsite-bron regelmatig; RLS aan, geen policies, dus alleen service_role), en checkVvvNews op de cron. De ophaal/parse/ontdubbel-logica is uit de handler getild naar fetchVvvNews() zodat endpoint en cron GEGARANDEERD dezelfde items zien; een tweede kopie zou onvermijdelijk gaan afwijken van wat de tab toont. Push gaat via de bestaande adminOnly-targeting (tag role=admin, v227), plus opts.url zodat de melding naar het originele artikel deeplinkt i.p.v. naar de app-root. UITSLUITEND kop + bron + link, geen AI-samenvatting: nieuws mag net zomin verzonnen worden als een kans. DRIE VANGNETTEN, elk tegen een stille fout die hier anders precies op 'niets nieuws' lijkt: (1) faalt ELKE bron (de clubsite gaf eerder 503 vanuit Cloudflare-IP's, v233), dan doen we geen uitspraak -- een lege lijst door een 503 leest anders eerst als 'niets nieuws' en zodra de bron terugkomt als 'alles nieuw'; (2) sb() geeft null bij een FOUT en [] bij 'geen rijen' -- die twee UIT ELKAAR gehouden, want een mislukte seen-lookup zou als 'nog nooit iets gezien' lezen en de hele feed pushen (zelfde familie als v251); (3) BURST-GUARD op 6 items/uur: de bronnen zijn scrapers, dus een gewijzigde URL-vorm maakt ALLES ineens 'nieuw' -- dat is geen nieuws maar een defect en van buitenaf niet van elkaar te onderscheiden, dus wegschrijven + EEN eerlijke waarschuwing i.p.v. een stortvloed of stilte. Ook: seed-run markeert de eerste vulling als gezien zonder te melden (anders 24 pushes in een klap), en boven de cap van 2 volgt een samenvattende melding i.p.v. losse. Wegschrijven gebeurt VOOR de push: mislukt de insert, dan zou de volgende run dezelfde items opnieuw melden -- liever een gemiste melding (gelogd) dan een meldingslus. Op de cron BEWUST als LAATSTE stap en buiten beide takken: buiten de takken omdat nieuws ook in de cron-gap-uren (23-05 UTC) verschijnt, en als laatste precies OMGEKEERD aan de Elo-sweep (v230/v239) -- die moest vooraan omdat hij essentieel is en de CPU-limiet hem anders afkapt, terwijl een nieuwsmelding nooit een scan mag verdringen; kapt de limiet hem af, dan probeert het volgende uur vanzelf opnieuw, want er wordt pas weggeschreven op het moment dat we ook echt melden. Venster 06-21 UTC = 08:00-23:00 NL: een bericht van 03:00 laat je telefoon niet trillen maar wordt om 08:00 alsnog gemeld i.p.v. stil verloren te gaan. Nul API-Football-calls. Raakt model/picks/CLV NIET. // // v252: DE RATE-LIMIT-VLAG GING VERLOREN ZODRA DE TWEEDE HOST ANTWOORDDE — v251 was daarmee een meter die zelf 0 kon melden terwijl het antwoord 'geweigerd' was. apif probeert twee hosts (api-sports, rapidapi). Wordt host 1 geweigerd, dan zet hij _rateLimited=true en valt door naar host 2; geeft die een andere fout of lege respons, dan loopt het naar `return out` (een kale []) en is de vlag weg — de `leeg`-constructie onderaan die de vlag aanhangt is ALLEEN bereikbaar als geen van beide hosts iets returnt. Zelfde gat als v245 dacht te dichten: ik zette de vlag toen op één uitgang en zag de andere over het hoofd. GEMETEN, niet geredeneerd: droogloop 16-07 04:00 UTC gaf fixturesGeweigerd=0 EN 0 fixtures, terwijl dezelfde call om 08:00 UTC 114 fixtures teruggaf waarvan 29 in league 2/3/848 (848=23, 3=6) — exact het aantal van de droogloop om 02:00. De data was er dus, de call gaf niets, de vlag zweeg. Nu: `if (!out.length && _rateLimited) out.rateLimited = true;` vóór return. HYPOTHESE DIE HIERMEE SNEUVELT: ik dacht dat apif's pagineerlus een geweigerde subpagina stil afkapte. Onjuist. /fixtures?date= is helemaal niet gepagineerd (114 results, paging.total=1) en `page` is niet eens een geldige parameter ('The Page field do not exist'), dus de v129-aanname klopt niet en `totalPages > 1` wordt nooit waar: het blok is dode code. BEWUST LATEN STAAN: het is een no-op (zou hij ooit vuren, dan haalt hij &page=2, krijgt de veldfout en breekt), en 'vuurt nooit' steunt op één meting van één dag met 3 competities. Op 20-07 draaien er 19; eerst kijken of /fixtures?date= dan wél meerdere pagina's meldt voordat ik hem weghaal. Minimale diff vlak voor de switch. Puur diagnostisch: geen drempel, formule of selectie gewijzigd. // // v251: EEN GEWEIGERDE FIXTURES-CALL WAS NIET TE ONDERSCHEIDEN VAN 'GEEN WEDSTRIJDEN' — zelfde familie als v249, maar een schakel hoger en daarmee erger. Gevonden doordat de droogloop van 16-07 na de v250-push '0 unieke fixtures gevonden' meldde terwijl dezelfde call een uur eerder 29 gaf; de /fixtures?date=-call was GEWEIGERD, niet leeg. apif hangt sinds v245 .rateLimited aan de lege array, maar noch runScan noch runScanTest las die vlag op dit pad. Dit is de meest stroomopwaartse call die er is: geen fixtures -> geen odds -> geen analyseBatch -> geen AI -> geen pick -> geen CLV, en het resultaat ziet er precies uit als een rustige speeldag. Nu: beide paden tellen de geweigerde date-calls, loggen 'GEWEIGERD ... dit is GEEN rustige dag', markeren het aantal als ONDERGRENS, en de 'geen wedstrijden'-CONCLUSIE wordt niet meer getrokken als de data niet is opgehaald (runScan logt dan een stop-waarschuwing; scan-test geeft verdict ONBRUIKBAAR + fixturesGeweigerd i.p.v. een geruststellend 'geen wedstrijden'). RAAKT DE GESCHIEDENIS: v238 concludeerde dat de avondscans van 20/21/22 UTC 'geen wedstrijden vonden' en noemde dat normaal omdat het WK voorbij was — die conclusie steunt op exact deze blinde !allMatches.length en is dus nooit hard geweest. Vanaf nu meetbaar. AANLEIDING OM DIT NU TE DOEN: op 16-07 03:45 UTC werd een ENKELE fixtures-call na minuten stilte geweigerd, dus dat is niet mijn testburst; vermoeden is dat snapshotOddsOnly (23-05 UTC) sinds v249 voor het eerst echte odds-calls doet i.p.v. instant leeg terug te komen, maar dat is een hypothese — en zonder dit onderscheid is hij niet te toetsen. Dat is precies het patroon van vannacht: wat de _cb-bug oploste was niet scherper redeneren maar v248, het meetbaar maken. Puur diagnostisch: geen drempel, formule of selectie gewijzigd. NOG OPEN: scan_status//health kennen dit onderscheid nog niet, dus een geweigerde fetch kan daar nog steeds als rustige dag ogen. // // v250: TWEEDE SEIZOENSBRON WEG — de tegenspraak die v243 wel signaleerde maar liet staan. runScanTest had SEASON_2026 (v113, 'identiek aan client seasonForLeague()') + getSeason(), die de top-5 op 2025 zette, terwijl leagueConfig in runScan alles op 2026 heeft. Gemeten bij de bron i.p.v. beredeneerd: /leagues?id=39 geeft seizoen 2026 = 2026-08-21 -> 2027-05-30 met current=true, en seizoen 2025 liep af op 24-05-2026. leagueConfig had dus gelijk en de set was verouderd. Weggehaald i.p.v. bijgewerkt: hij werd nog op precies EEN plek gebruikt — een logregel — dus bijwerken zou de tegenspraak alleen verplaatsen tot de volgende seizoenswissel, en dat is exact hoe deze set stil kon verouderen. Enige echte bron is f.league.season uit de fixture zelf; dat is ook wat fetchOddsForFixtures sinds v243 gebruikt om de (datum, competitie, seizoen)-combinaties te plannen. De logregel staat nu NA de fixture-fetch en rapporteert het seizoen dat de scan echt gaat gebruiken, met 'ONTBREEKT' als het leeg is — dat zou betekenen dat de per-competitie-route die competitie overslaat en hij stil in de datum-bulk belandt. CORRECTIE OP MEZELF: ik meldde eerder dat de scan-test hierdoor 'mogelijk het verkeerde seizoen test'. Dat was onjuist. getSeason zat in geen enkele API-call — runScanTest haalt fixtures met een globale /fixtures?date= en filtert in code. Ik had het patroon herkend (twee bronnen die elkaar tegenspreken) en de impact aangenomen zonder te kijken waar de variabele landt; dezelfde fout als in de v240-comment over poissonMap. Het opruimen blijft juist, maar om de val weg te nemen, niet om een bug te fixen. seasonOf() in runScan (r2802) blijft: die zit wel in een echte call (/fixtures?league=&season=&next=10) maar uitsluitend voor NEXT_LEAGUES, en die verzameling heeft nul overlap met de 19 FASE 2-competities — hij raakt dus alleen FASE 1 (WK/Eliteserien/Allsvenskan), waar zijn waarde correct is. Geen gedragswijziging: puur een dode duplicaat eruit + een eerlijker logregel. // // v249: DE ODDS-BULK HEEFT SINDS v152 NOOIT GEWERKT — GRONDOORZAAK VAN DE 1/24. Gevonden doordat v248 de dekking meetbaar maakte: de droogloop gaf 0 rate-limits, 8 calls, 2 geplande combinaties en toch 1/24, terwijl bij de bron 16 fixtures odds hadden waarvan 12 IN de batch zaten. De data kwam dus binnen bereik en verdween. Oorzaak: fetchOddsForFixtures bouwde `&_cb=${Date.now()}` (v152, 'cache-bust op odds fetch') en apif() gebruikt het pad RECHTSTREEKS — de _cb-strip zat alleen in handleAPIFootball, de proxy. API-Football weigert onbekende velden: aangetoond met _cb=abc (niet-numeriek, ontsnapte aan de strip-regex /[&?]_cb=\d+/) -> {"_cb": "The _cb field do not exist."}. Dus response=[] -> out=[] -> `if (!r?.length) break` -> NUL odds. Geen fout, geen rate-limit, exact zoals 'geen data'. De enige odds die ooit binnenkwamen kwamen uit de individuele fallback (/odds?fixture=, zonder _cb, max 5) en de goal-lus — vandaar stelselmatig ~1 van 24. De cachebuster kocht bovendien niets: apif zet op elke fetch al cf:{cacheTtl:0,cacheEverything:false}. Dit werpt v242 om: ik zag toen 1/24 op twee verschillende competities, concludeerde terecht 'geen dekkingsprobleem maar een bug' en repareerde de PAGINATIE — die kon per definitie niets doen omdat de call nooit data teruggaf. Juiste conclusie, verkeerde oorzaak, en de echte stond twee regels verderop. v243 (per competitie) en v245/v247 (rateLimited-vlag) bouwden allemaal boven op een call die structureel leeg terugkwam. Ook mijn eigen throttle-hypothese van vanavond sneuvelt hiermee: 0 rate-limits in de scan-run zelf; wat ik voor een burst-throttle aanzag waren mijn eigen curl-calls. Nu: (1) _cb weg uit beide odds-lussen; (2) vangnet in apif dat _cb strip't — BOVEN de hosts-array, want die bakt het pad in de URL (mijn eerste poging stond eronder en deed dus niets); (3) de proxy-strip van /[&?]_cb=\d+/ naar /[&?]_cb=[^&]*/g, want een niet-numerieke waarde ontsnapte daar nog. Bewust alleen _cb: elke andere onbekende parameter hoort hard te falen. Geen drempel of formule gewijzigd — dit herstelt alleen de odds-toevoer, en die is de voorwaarde voor ELKE pick: zonder odds geen analyseBatch, geen AI-analyse, geen pick, geen CLV. Verwachting op de droogloop: dekking van 1/24 naar ~12/24. // // v248: DE ODDS-DEKKING MEETBAAR MAKEN I.P.V. AFLEIDEN. Droogloop 16-07 (/scan-test?league=2,3,848&goals=1, v247) gaf 1/24 dekking, exact het cijfer waarop v242 de paginatiebug diagnosticeerde -- na v242 EN v243 staat het er dus nog steeds. Rechtstreeks bij de bron gemeten: de odds BESTAAN (league 3 = 6 fixtures, league 848 = 10 results over 3 pagina's ~26; league 2 = 0, klopt want CL-kwalificatie loopt later), en de fixture-ids uit die odds (1554416/17, 1554447/48/49) zitten aantoonbaar IN de batch. De data komt dus binnen bereik en verdwijnt daarna. Meest waarschijnlijke oorzaak is de burst/IP-throttle op de gedeelde Cloudflare-egress die v244 al mat (weigering bij een handjevol calls per minuut terwijl het dagverbruik ~100/7500 is) -- v245 heeft die alleen ZICHTBAAR gemaakt, niet opgelost, en v243 zette maxCalls juist 30->80 wat de burst verergert. MAAR: dat is een afleiding, geen meting, en ik heb vanavond al drie keer een patroon herkend en de conclusie vooruit laten lopen op het bewijs (v240-comment over poissonMap; de seizoen-tegenspraak die cosmetisch bleek; en een '_cb breekt de call'-hypothese die ik 'bevestigd' noemde op grond van een omgekeerde-volgorde-test die de intermitterende throttle niet KON uitsluiten -- _cb wordt door cleanPath gestript en bereikt de API nooit). Dus eerst meten. De rlHits-telling bestond al (v245/v247) maar ging uitsluitend naar console.log en was daarmee buiten de Cloudflare-logs om onzichtbaar -- precies de blinde vlek die dit zo lang liet duren. Nu: fetchOddsForFixtures accepteert een optioneel `stats`-object (zesde parameter; de drie bestaande aanroepers geven het niet mee en veranderen niet van gedrag) dat pad, calls, geplande combinaties, dekking en de rate-limits per lus vult. BEWUST een aparte parameter en GEEN veld op oddsMap: die map wordt op 8 plekken met Object.keys().length geteld en in runScan met Object.entries() geitereerd, dus een vlag daarin komt binnen als een wedstrijd met odds -- dat was de v245-bug die v246 opruimde. runScanTest geeft de telling terug in log + oddsStats, zodat de volgende droogloop zwart-op-wit laat zien of de 23 ontbrekende fixtures GEWEIGERD zijn of echt geen odds hebben. Pas als dat vaststaat volgt de oplossing (serialiseren met pauze, maxCalls terug, of andere egress). Puur diagnostisch: geen drempel, formule of selectie gewijzigd. // // v247: DRIE STILLE FOUTEN UIT MIJN EIGEN v242-v246-REEKS. Die reeks is nooit op een echte speeldag geverifieerd (5 van de fixes repareerden een fout uit een vorige fix), dus koud nagelopen op de familie 'stille fout' vlak voor de competitieswitch van 20-07. (1) poissonMap deed `{h: ai.h || 33, x: ai.x || 33, a: ai.a || 34}` -- ACHTSTE instantie van de falsy-fallback-familie, en mijn eigen v240-comment beweert nog dat deze lus het 'al goed deed met if (!ai) return'. Dat klopt voor het OBJECT, niet voor de VELDEN: ik heb het objectniveau geverifieerd en het veldniveau aangenomen. Een onvolledig antwoord ({h:62} zonder x/a -- exact wat een afgekapt of half geparsed JSON-object geeft, en met max_tokens 2048 nog ongetest op volle batches) werd stilzwijgend {h:62,x:33,a:34} = 129%, geen geldige kansverdeling; h:0 is bovendien een geldige kans die naar 33 werd getild. Niet cosmetisch: poissonMap voedt detectSharpMoney -> divergentie -> sharpScore, die de longshot-guard gate't (odds>=3.5 vereist sharpScore>=55) en sharp_signal_results/market_consensus vult. Nu Number.isFinite op h/x/a met overslaan + telling in de log. (2) De rateLimited-vlag van v245 werd op maar EEN van de twee pagineerlussen gelezen: de datum-bulk-terugval had nog het kale `if (!r?.length) break`. Dat is niet de dode tak -- snapshotOddsOnly heeft geen seizoen per wedstrijd en valt dus ALTIJD daar binnen, en dat is juist de 23:00-05:00-run die de SLOTKOERS vormt: de ruggengraat van CLV. Een burst-throttle las als 'laatste pagina' -> slotkoers viel stil weg. Exact de bug die v245 dacht te dichten, half gerepareerd. (3) De losse odds-fallback en de goal-lus telden een geweigerde call als 'leeg' -- zichtbaar maar op een hoop met echte lege respons; nu apart geteld en gelogd als ONVOLLEDIG. Geen enkele drempel of formule gewijzigd: dit haalt alleen verzonnen data en stille onderdekking eruit. Familie 'vlag als sleutel' (de v245-bug uit v246) apart nagelopen: geen verdere instanties -- alle maps worden uit rij-data met echte id's als sleutel opgebouwd. // // v246: GEEN HERANALYSE BIJ STILSTAANDE KOERS + batch-verhongering opgelost. Aanleiding: Robs vraag of elk uur scannen wel zinvol is. Gemeten in Supabase: 64% van de opeenvolgende odds-metingen is ONGEWIJZIGD, en de scan-prompt bevat per wedstrijd alleen namen/competitie/datum/1X2-odds op temperature 0 -- identieke odds geven dus gegarandeerd een identiek antwoord. Twee van de drie heranalyses waren letterlijk hetzelfde. Dat kostte geen geld (12 gebundelde calls/dag, ~$5/mnd) maar wel BATCH-PLEKKEN: analyseBatch was een deelverzameling van batch (soonest-first 24), dus op een zaterdag met 50 duels werden elk uur dezelfde 24 opnieuw gedaan terwijl 25+ moest wachten tot de eerste waren afgetrapt -- de logregel beweerde dat ze 'doorschuiven naar de volgende uur-run', wat aantoonbaar onwaar was omdat die run opnieuw dezelfde kop pakt. Nu: selecteerTeAnalyseren() kijkt naar ALLE duels met odds en houdt alleen over wat (a) nooit geanalyseerd is, (b) bewogen odds heeft, of (c) ouder is dan 6 uur -- (c) omdat het Poisson-deel wel op teamstatistieken leunt die na gespeelde duels schuiven terwijl de koers stil kan staan. Nieuwe tabel ai_analysis_log (fixture_id PK + de odds waarop de analyse draaide) bevat BEWUST geen AI-uitkomst: niets wordt hergebruikt, alleen overgeslagen -- geen spanning met CIJFERBRON, en geen aanname dat een antwoord losstaat van de batch-context waarin het ontstond. Faalt de lookup, dan analyseren we alles (oud gedrag). Alleen loggen wat de AI echt terugggaf, anders zou een afgekapte call duels als 'gedaan' markeren. Snapshots blijven elk uur doorlopen -- die zijn de ruggengraat van CLV en daar zit de waarde van uurlijks scannen. NB: hierbij ook mijn eigen v245-bug opgeruimd: `oddsMap._onvolledig = comboFouten` zette een vlag ALS SLEUTEL in de oddsMap, terwijl op 8 plekken Object.keys(oddsMap).length wordt geteld en runScan Object.entries(oddsMap) itereert -- die vlag zou daar als wedstrijd met odds zijn binnengekomen. // // v245: EEN GEWEIGERDE CALL WAS NIET TE ONDERSCHEIDEN VAN 'GEEN DATA' -- gat dat ik in v242/v243 zelf inbouwde. Gemeten met de headers uit v244: de per-minuut-limiet is 300 (niet de 450 die ik uit mijn geheugen citeerde), maar api-sports weigert bursts terwijl de teller nog op 293-299/300 staat, en die weigeringen dragen GEEN rate-limit-headers. Het is dus een burst/IP-throttle -- gedeelde Cloudflare-egress, zelfde familie als de Google News-503 uit v233 -- en geen quotaprobleem; dagverbruik was 120/7500. Gevolg: apif() gaf na opgegeven retries [] terug, exact zoals bij 'geen data', en mijn pagineerlus las `if (!r?.length) break` -> een geweigerde call las als 'laatste pagina' -> dekking viel stil weg zonder enige fout. Precies de bug die ik dacht te dichten. Nu: apif hangt een .rateLimited-vlag aan de lege array (bestaande aanroepers zien nog steeds length 0 en veranderen niet van gedrag), retries 2 -> 3 met oplopende backoff, en de odds-lus telt afgebroken combinaties apart en logt 'dekking is ONVOLLEDIG, niet geen odds'. Ook: de PROXY retryde helemaal niet bij een rate-limit en gaf de fout rechtstreeks door aan de frontend -- dat is het 'model n.v.t.' dat v216 al beschreef (de statistics-calls die het Poisson-model voeden vielen weg). Interne apif deed dit al sinds v114; de proxy bleef achter. Nu daar dezelfde backoff-retry. // // v244: rate-limit-headers van API-Football doorgeven i.p.v. weggooien (diagnose). Aanleiding: ik liep bij ~8 calls per minuut al op 'Too many requests' terwijl het dagverbruik 102/7500 was -- dat rijmt niet met de 450/min die Pro hoort te hebben, en het is niet academisch omdat v243 op een drukke dag tot 80 odds-calls in een burst vuurt. De proxy gooide de headers weg, waardoor de limiet alleen een symptoom was zonder cijfer en ik hem uit mijn geheugen citeerde. Nu doorgegeven als X-APIF-Minuut (uit x-ratelimit-limit/remaining), X-APIF-Dag (uit x-ratelimit-requests-limit/remaining) en X-APIF-Host, zodat meetbaar is welke limiet echt geldt en via welke host. Puur diagnostisch: geen gedrag gewijzigd, alleen response-headers toegevoegd. // // v243: ODDS PER COMPETITIE + DEKKINGS-ALARM. (1) v242 liet de datum-bulk doorpagineren met een plafond van 12, maar dat is niet genoeg: 15-07 -- ongeveer de rustigste dag van het jaar, tussen WK en clubseizoen -- had al ACHT pagina's, dus een zaterdag in het seizoen loopt gegarandeerd tegen het plafond en dan is de dekkingsbug terug. Niet te meten ook: API-Football gooit odds weg zodra een wedstrijd gespeeld is, dus een drukke zaterdag is niet na te bootsen (historische datums geven allemaal paging=1). Bovendien is >90% van die wereldlijst ballast die wij niet scannen. Nu vraagt fetchOddsForFixtures alleen de (datum, competitie)-combinaties op waar we ECHT wedstrijden hebben: begrensd door ons eigen programma i.p.v. door het wereldaanbod, en elk resultaat is relevant. Gemeten: per competitie geeft CL 1 pagina, EL 1, ECL 3. maxCalls 30->80 in runScan (drukke zaterdag ~15 competities x 2 datums x 1-2 pagina's); 12 scans x 80 = ~960 odds-calls/dag, ruim binnen de 7500 van Pro (gemeten verbruik vandaag: 77). Het seizoen komt uit de fixture zelf (f.league.season) i.p.v. uit een tabel: leagueConfig (alles 2026) en de SEASON_2026-set in runScanTest (top-5 -> 2025) spreken elkaar TEGEN, en /odds weigert zonder season ('The Season field is required'). Zonder bruikbare matches (snapshotOddsOnly heeft geen seizoen) valt hij terug op de datum-bulk van v242. (2) DEKKINGS-ALARM: scan_status.last_with_odds/last_without_odds werden al weggeschreven, maar v_health geeft ze niet door en niemand keek ernaar -- daarom kon 1-van-24 onzichtbaar blijven. /health leest scan_status nu rechtstreeks en toont altijd odds_dekking {met_odds, van, pct}; waarschuwing odds_dekking_laag zodra <15% bij >=10 wedstrijden. Drempel bewust laag: domestic hoort 80-100% te halen, maar kwalificatierondes met kleine clubs halen echt ~20% (gemeten 5/24), en een vangnet dat permanent rood staat wordt genegeerd (v224). Hertunen op clubdata. // // v242: ODDS-BULK LAS MAAR TWEE VAN DE ACHT PAGINA'S -- launch-kritiek voor 20-07. Gevonden tijdens de droogloop die v241 mogelijk maakte: /scan-test op zowel de CL/EL/ECL-kwalificaties (leagues 2,3,848) als op clubvriendschappelijke duels (667) gaf allebei exact 1 van 24 fixtures met odds. Twee totaal verschillende competities met hetzelfde cijfer = geen dekkingsprobleem maar een bug. Rechtstreeks gemeten bij de bron: /odds?date=2026-07-15 geeft results=10 met paging.total=8 (16-07: 6 pagina's), dus ~80 fixtures met odds -- maar fetchOddsForFixtures haalde hardgecodeerd pagina 1 en 2 op, onder een comment dat beweerde 'alle odds voor die dag in 1 call'. De paginering was wel opgemerkt en niet uitgevoerd. Dekking was dus ~20 van ~80, plus een individuele fallback van 5. Tijdens het WK onzichtbaar (handvol duels per dag, picks_total=18), maar vanaf de competitieswitch met 19 competities en ~50 fixtures per dag zou het merendeel stil zonder odds vallen -- en zonder odds komt een wedstrijd niet in analyseBatch, dus geen AI-analyse, geen pick. Een scan die niets vindt ziet er precies uit als een efficiente markt. Nu doorlopen tot de data op is: stop bij een lege pagina, bij een pagina korter dan ODDS_PAGE_SIZE (=laatste), of zodra alle fixtures gedekt zijn; maxCalls (30 in productie) blijft de harde grens en dekt 2 datums x 8 pagina's ruim. Plus een dekkings-logregel, want dit gat was alleen zichtbaar doordat scan-test het toevallig per fixture logde. apif pagineert zelf alleen /fixtures?date= (v129, PAGE_CAP 5) -- odds vielen daarbuiten en dat blijft zo, de paginering zit hier waar de dekkingsconditie bekend is. // // v241: /scan-test kon de productie-batch niet nabootsen. De testroute cápte hardgecodeerd op 10 wedstrijden terwijl productie MAX_AI_ANALYSES_PER_SCAN=24 gebruikt. Een testpad dat de productie-batchgrootte niet haalt, kan batchgrootte-afhankelijke fouten per definitie nooit aantonen -- en dat is exact waarom de max_tokens-afkapping uit v240 hier nooit boven water is gekomen: elke test bleef onder de drempel waar het misgaat. Nu dezelfde constante als productie, plus ?max=N (begrensd op 40) om bewust te over-schrijven. runScanTest kreeg daarvoor een maxCap-parameter (het had geen toegang tot de request-URL). Bedoeld voor de droogloop vóór de competitieswitch van 20-07: /scan-test?league=2,3,848&goals=1 geeft rond 16-07 een volle batch uit de CL/EL/ECL-kwalificaties. runScanTest schrijft niets naar picks/shadow/CLV -- alleen kostenregistratie onder uid scan-test -- dus een droogloop vervuilt geen enkele dataset. // // v240: SCAN VERZON KANSEN BIJ EEN KORT AI-ANTWOORD + max_tokens was een tijdbom op 20-07. Gevonden bij Robs vraag naar de AI-kosten van de scan; de kosten bleken een non-issue (gemeten: cron-scan 148 calls / $0,487 totaal = $0,0033 per call, en het is EEN gebundelde call per scan, niet 24 losse -- mijn eerdere waarschuwing over '~288 analyses per dag' was onjuist), maar ernaast lagen twee echte fouten. (1) `aiResults[i] || {h:50,x:25,a:25,c:5}` in de pick-lus: gaf de AI minder objecten terug dan er wedstrijden waren, dan kreeg elke resterende wedstrijd een VERZONNEN 50/25/25. Directe schending van CIJFERBRON, en niet ingeperkt: c.aiKans voedt de candidate-vorming, model_market_comparison (v_ai_tip_accuracy) en shadow_picks -- de datasets van de draw-evaluatie (eind aug) en de Elo-blend-validatie (okt). De c:5 remt niets, want calculateConfidenceV20 gebruikt ai.c niet eens en leunt voor 40% op de markt; een verzonnen 50/25/25 haalt de conf-drempel dus gewoon. De poissonMap-lus 30 regels hoger deed het al goed met `if (!ai) return` -- de pick-lus week daar zonder reden van af. Zevende instantie van de falsy-fallback- familie. Nu: overslaan (geen data = geen pick, geen shadow-rij, geen tip), met expliciete Number.isFinite-check op h/x/a i.p.v. truthiness, want 0 is een geldige kans. (2) max_tokens stond op 512 terwijl een volle batch 24 JSON-objecten moet opleveren (met goal-markten ook gh/ga). Nooit geraakt omdat het WK 1-4 duels per scan gaf (gemeten: 62 output-tokens gemiddeld), maar vanaf 20-07 met 19 competities zijn volle batches de norm -> afgekapt antwoord -> regex vindt geen sluitende array -> aiResults leeg -> voorheen kreeg ELKE wedstrijd 50/25/25. De twee bugs versterkten elkaar dus precies op de competitieswitch. 512 -> 2048 in hoofdscan en scan-test; kost niets, Anthropic rekent af op werkelijke output. (3) Gedeeltelijke antwoorden loggen nu expliciet -- die zien er geslaagd uit en zijn daarmee de gevaarlijkste variant. // // v239: ELO-SWEEP DRAAIDE NOOIT AUTOMATISCH. Gevonden via de resterende /health-waarschuwing elo_sweep_verouderd(last=2026-07-13): sweepEloFromResults stond sinds v209 alleen in het /settle-endpoint, en niets roept /settle aan -- niet de cron-handler (die doet runScan/verify/settleShadow/settleAhShadow/settleModelTips) en geen van de vier workflows. De sweep liep dus uitsluitend handmatig; de marker stond op 13-07 omdat ik hem toen tijdens het v230-werk zelf aanstootte. Impact was niet cosmetisch: team_ratings/elo_history rijpen niet, n_duels blijft 0, en de Elo/SoS-blend-validatie van medio oktober (drempel n_duels>=40) kon zo per definitie nooit slagen -- met de launch in de winterstop is dat juist het onderdeel waar nu tijd voor is. Pijnlijk detail: v228 bouwde observability en v230 zette de sweep vooraan tegen de CPU-limiet, allebei voor een pad dat niet eens gepland stond; het v228-vangnet heeft het uiteindelijk wel netjes zelf aan het licht gebracht. Nu aangeroepen als EERSTE stap in de fullScan-tak van de cron, om dezelfde reden als v230: de sweep is licht, runScan is zwaar, en erna zou de CPU-limiet hem afkappen. De functie throttlet zichzelf op 1x/dag (model_config.elo_last_sweep), dus de overige 11 scans kosten alleen een marker-read, en elk uur aanroepen levert gratis een retry als de 06:00-run faalt. Raakt picks NIET: de blend blijft dormant op elo_blend_w=0. // // v238: SCAN-HARTSLAG GEREPAREERD -- de 'dode cron' van 14-07 was geen dode cron. Aanleiding: /health stond om 03:10 op last_scan=14-07 19:01 (488 min) en scan_version=v233, wat op stilgevallen crons leek. Gemeten i.p.v. aangenomen: een handmatige /scan-now gaf direct ok=true met scan_version=v237 en snapshots_total liep 's nachts gewoon door (831->832), dus de crons vuurden altijd al. Werkelijke oorzaak: MAX_SCANS_PER_DAY=8 terwijl de cron 12 volledige scans per dag plant (0 6 + 0 12-22). De scans van 20/21/22 UTC vonden geen wedstrijden (WK voorbij, clubseizoen start 20-07), belandden in de !allMatches.length-tak, sloegen op de limiet aan en keerden terug ZONDER lastRun weg te schrijven -- dus bevroor last_scan op 19:01 en riep /health 'scan_verouderd' (en de workflow zou bij >400 min 'cron lijkt dood' melden) terwijl er niets aan de hand was. De poort deugde op drie punten niet: hij stond alleen in het GOEDKOPE pad (geen AI) terwijl het dure pad met AI-analyses helemaal geen dagcap heeft (omgekeerd), hij zat NA de fixtures-fetch en bespaarde dus geen enkele API-call, en het enige effect dat overbleef was het overslaan van de hartslag. Poort verwijderd: lastRun wordt nu altijd geschreven. Dat een scan niets vond blijkt uit lastMatchCount=0, niet uit een ontbrekende regel. De teller blijft lopen voor de /scan-now-cap (40, v160/v189). Mijn eerdere hypothese dat mijn eigen deploy-reeks de 20:00-scan afkapte was FOUT en is hiermee weerlegd. Raakt model/picks/CLV NIET. // // v237: HEALTH-MONITOR GING AF OP MIJN EIGEN DEPLOYS. Run #54 faalde op versie_mismatch(scan=v233,worker=v236) na de v235/v236-pushes: de worker was nieuw, de laatste scan (19:01) liep nog onder de vorige versie. Dat is direct na een deploy volkomen normaal, maar de check zette ok=false -> ELKE worker-deploy maakte de monitor rood tot de volgende volledige scan. Overdag maximaal een uur, maar na 22:00 UTC draait de eerstvolgende fullScan pas om 06:00 -> 7+ uur rood en een nacht vals alarm. Een vangnet dat op eigen deploys afgaat wordt genegeerd en vangt daarna niets meer -- exact de alarmmoeheid die v224 al voor clv_gaps moest repareren. De check bedoelde: 'worker gedeployd maar de scan draait nog oude code'. Precieze test daarvoor: is er SINDS deze workerversie al een volledige scan geweest? Zo ja en die rapporteert nog de oude versie, dan is de deploy niet doorgekomen -> echt alarm. Zo nee, dan wachten we gewoon. Nieuwe marker model_config.worker_version_seen (note=VERSION, updated_at=eerste keer gezien; zelfde upsert-patroon als elo_last_sweep) levert dat moment. Wachtstand is niet stil: hij komt als versie_info='wacht_op_scan(...)' in het antwoord, alleen zonder ok=false. Is de marker onbereikbaar, dan valt hij terug op waarschuwen (liever vals alarm dan een gemiste kapotte deploy). Raakt model/picks/CLV NIET. // // v236: Google News-bron er weer uit, op grond van de productiemeting die v235 mogelijk maakte: het sources-veld gaf {'GoogleNews': 'fout: HTTP 503'}. Google blokkeert Cloudflare-Worker-IP's nog altijd -- tweede onafhankelijke bevestiging na v233, dus beleid en geen hapering. Een bron die gegarandeerd faalt kost per cache-miss een subrequest en vertraagt de tab, zonder ooit een item te leveren. De allSettled-opzet deed wel exact zijn werk: de uitval was zichtbaar en onschadelijk (de andere twee leverden gewoon 22 items) -- daarom kon dit als experiment in productie i.p.v. als gok. Conclusie vastgelegd in een comment zodat niemand het over een half jaar opnieuw probeert: eerst een 200 vanaf een Worker aantonen. /vvv-news blijft op twee bronnen: vvv-venlo.nl + Venlonaren.net. // // v235: Google News als DERDE nieuwsbron in /vvv-news, naast vvv-venlo.nl en Venlonaren.net. Toegevoegd na meting, niet op gevoel: de Google News-RSS voor "VVV-Venlo" levert regionale media die de andere twee missen (L1 Nieuws 8x, Omroep Venlo 4x, De Limburger 3x, ESPN 2x). Robs eigenlijke vraag (Voetbalprimeur/Voetbalzone) lost dit NIET op: gemeten 0 items van beide in die feed. Voetbalzone heeft geen RSS en blokkeert ClaudeBot/anthropic-ai in robots.txt; Voetbalprimeurs RSS is algemeen (24 items = 3 uur venster, 0x VVV) en hun clubpagina hangt aan Next.js-hashclasses (NewsListItem_time__OpRog) die per deploy wijzigen -> stil brekende scraper, bewust niet gebouwd. Sofascore geeft 403 op alles incl. robots.txt en is bovendien een stats-site, geen nieuwsbron (dat dekt API-Football Pro al, gelicenseerd). Items van VVV-Venlo/Venlonaren worden in deze bron OVERGESLAGEN -- die halen we rechtstreeks op met een echte artikel-link i.p.v. een news.google.com-omleiding, wat tevens het ontdubbelen oplost (Google-links zijn per definitie andere URL's). Titelfilter op vvv|venlo omdat de query ook de bodytekst matcht. v233 haalde Google News eruit wegens 503 vanuit Cloudflare-IP's; die 503 is vanuit een Worker niet vooraf te meten, dus staat de bron in dezelfde Promise.allSettled: faalt hij, dan meldt het sources-veld dat en blijven de andere twee draaien. Raakt model/picks/CLV NIET. // // v234: /vvv-news haalt nu TWEE bronnen op i.p.v. alleen de officiele clubsite: vvv-venlo.nl/nieuws (HTML, ongewijzigd) EN Venlonaren.net (/feed/, echte WordPress-RSS). Reden: het supportersplatform publiceert dagelijks over VVV terwijl de clubsite weinig en traag post. Gemeten voor de bouw: robots.txt van venlonaren.net staat alles toe behalve /wp-admin/, /feed/ geeft 200 application/rss+xml met 10 verse items, parser getest tegen de echte feed (10/10 titels+datums+links correct). We nemen UITSLUITEND kop + datum + link over, NOOIT de artikeltekst -- de feed levert content:encoded (volledig artikel), maar dat overnemen zou hun werk herpubliceren en het verkeer weghalen bij een site die er juist van leeft; tikken opent de bron. Beide bronnen draaien via Promise.allSettled, dus een 503 bij de een laat de ander gewoon door (de clubsite gaf eerder 503 vanuit Cloudflare-IP's, zie v233 -- dat mag de tab niet meer leegtrekken). Ontdubbeling op link, sortering nieuwste-eerst waarbij items ZONDER datum achteraan komen i.p.v. bovenaan (een lege datum mag geen vers nieuws voorwenden), cap 24. Respons kreeg een sources-veld {bron: aantal|fout} zodat een stille bronuitval zichtbaar is i.p.v. als 'minder nieuws' te verschijnen. Raakt model/picks/CLV NIET. // // v233: /vvv-news haalt nu de OFFICIELE VVV-Venlo-nieuwspagina op (Google News gaf 503 vanuit Cloudflare-IP's) — titel+datum+link per artikel, officiele bron. // v231: /vvv-news endpoint. // v230: Elo-sweep in /settle als EERSTE stap i.p.v. laatste. De automatische /settle kapte de sweep af door de Cloudflare CPU-tijdlimiet (marker bevroor sinds 11-07 zonder fout; handmatig triggeren werkte wel). Sweep is licht (vaak alleen de marker-write), dus vooraan draaien garandeert dat 'ie voltooit; het zware shadow/AH-settle-werk komt erna. Nul pick-impact (elo dormant). // v229: KKD-MISLABEL GEFIXT + VVV-Venlo's competitie toegevoegd. league 43 stond op drie plekken (leagueConfig FASE 2, activeLeagueIds-spiegel, LEAGUE_FACTORS) gelabeld als 'Keuken Kampioen Divisie', maar 43 = National League (Engeland, 5e divisie); de echte KKD is 89. Gevolg: post-20/07 zou een Engelse 5e divisie gescand worden en VVV's competitie helemaal niet. Nu overal 43->89 (factor 0.90 ongewijzigd, past bij een 2e divisie). Blijft 19 competities. Frontend ongewijzigd: COMP_IDS zet eredivisie(88)+kkd(89) al bovenaan en kent 89 al als KKD (oefenduels-pad). Mislabel-oorsprong: v185 ('factoren toegevoegd voor .../79/43/80/41'). // v228: ELO-SWEEP OBSERVABILITY. sweepEloFromResults bevroor stil sinds 08-07 (elo_history 14 rijen, marker elo_last_sweep.note vast op 08-07) terwijl /settle bleef draaien (shadow+AH settelden vandaag) — de fout werd DUBBEL weggeslikt: de outer catch van de sweep en de eigen try/catch van updateEloForFixture loggen enkel console.error, dus 3 dagen stille freeze zonder alarm. Nul pick-impact (elo dormant, elo_blend_w=0), maar het SoS-fundament rijpte niet richting de okt-blend-validatie. Nu: markEloSweep zet model_config.elo_sweep_status='ok' bij succes; de outer catch schrijft de ECHTE foutmelding naar diezelfde rij ('ERR: ...') zodat de oorzaak op de eerstvolgende /settle zichtbaar wordt (config_key is PK, upsert geverifieerd). /health leest elo_last_sweep.note + elo_sweep_status en pusht elo_sweep_verouderd(last=X) zodra de marker ouder is dan gisteren, plus elo_sweep_fout(...) als de status een ERR bevat — zelfde vangnet-patroon als clv_gaps (v220) en rls_gat (v206), de health-monitor pikt het automatisch op. Raakt picks NIET. // v226: /model-params endpoint. De app toont als 'value' het RUWE model-markt-verschil, maar de pickselectie beslist op calculateValue() = (1-w) x ruw, met w = MARKET_SHRINK (0.45 club / 0.55 toernooi) + TUNE.s1/s2 (extra shrink onder 20% resp. 20-35% modelkans, alleen 1X2), en voor een gelijkspel nog x0.88. Gemeten op Spain-Belgium: ruw 4.8pp -> opgeslagen value_pct 1.8 (factor 2.7). Daardoor leek +4.7pp op de draw ruim boven de '3pp value'-uitleg, terwijl de echte drempel voor een toernooi-draw 7pp NA correctie is -- vandaar geen pick en zelfs geen shadow-rij. Endpoint levert shrink, tune, drawPenalty, tournamentLeagues, minValue en longshotOdds zodat de frontend met DEZELFDE formule rekent i.p.v. de constanten te dupliceren (drift zodra autotune s1/s2 bijstelt). Raakt geen enkele berekening. // v225: ENABLE_GOAL_MARKETS naar de top verplaatst (stond op r1617, werd op r157 door snapshotOddsOnly gelezen). Runtime veilig, maar eslint no-use-before-define -- nu als blokkerende pre-push-check -- kan een veilige forward-reference niet onderscheiden van de TDZ-bug die in v26.265 elke AH-analyse brak. Declaratie boven gebruik. Geen gedragswijziging. // v224: ALARMMOEHEID VOORKOMEN. De settled_zonder_clv-waarschuwing uit v220 zette ok=false, en omdat het gat (Mexico-England 06-07) BEWUST open blijft om de dataset eerlijk te houden, faalde de dagelijkse health-monitor voortaan elke dag (run #34). Een vangnet dat permanent rood staat wordt genegeerd en vangt daarna niets meer. Nu een dismiss-drempel model_config.clv_gaps_ack (zelfde patroon als elo_blend_validated): waarschuwen zodra clv_gaps > ack. Het veld clv_gaps blijft altijd zichtbaar in /health. NIEUWE gaten alarmeren dus wel. Ontwerpfout van mezelf, niet van de monitor. // v223: league_calibration opgeschoond + de bron van de vervuiling gedicht. (1) Twee rijen verwijderd (backup: league_calibration_backup_v223.json): league_id '10' Friendlies met 10 picks/5 wins die in `picks` NIET bestaan en avg_value 97.54 (onmogelijk in pp) -> via expectedHitrate klemde factor op de ondergrens 0.70, wat na 20-07 elke clubvriendschappelijke pick met ~30% zou dempen; en league_id 'unknown' (La Liga, 108 picks, factor 1.022). (2) updateLeagueCalibration deed `String(pick.leagueId || 'unknown')` -> elke pick zonder league-id landde in die bucket en erfde haar kalibratie. Nu expliciete undefined/null/''-check: overslaan + console.warn. DEZELFDE bucket zat ook in runWeeklyCalibration (r2398), die de verwijderde rij elke zondag opnieuw had aangemaakt — beide gedicht. Zesde instantie van de falsy-familie. factor telt voor 70% mee in leagueFactor en vermenigvuldigt de score, dus dit was GEEN cosmetische data. Overgebleven rijen (WK 10, Primera 7, J1 1) zijn exact gedekt door `picks`. // v222: BUGFIX League Ratings ROI. leagueTiers stuurde league_calibration.roi rechtstreeks door, maar dat veld is de SOM van unit-rendementen x100 (WK: 271.00 over 10 picks), geen percentage. hitrate werd hier wel door total gedeeld, roi niet -> de app toonde Primera Nacional +77.0% waar de picks-tabel +11.0% zegt, en WK +271%. Bij 1 pick viel het niet op omdat som==gemiddelde (J1 +130% klopte toevallig). Nu roi/total. Raakt de pickselectie NIET; league_calibration.factor (die wel meeweegt, 70% van leagueFactor) blijft ongemoeid. // v221: /report endpoint (POST) -> tabel content_reports (RLS aan, alleen service_role). Google Play's AI-Generated Content-policy eist een IN-APP meldweg voor aanstootgevende AI-output, zonder de app te verlaten; de analyse-narratief wordt door de LLM geschreven, dus dit is van toepassing. Velden geclipt (reason 80, details 2000, match_name 200, UA 300) tegen misbruik. Raakt picks/model NIET. // v220: (a) AH-CURVE GEPERSISTEERD. parseGoalConsensus bouwde odds.ah al (v205/v213) uit dezelfde per-fixture call, maar saveOddsSnapshots schreef alleen ou+btts weg -> ah_shadow_picks kon per definitie GEEN CLV krijgen, precies de metriek die moet beslissen of AH uit shadow mag. Nu goal_odds.ah = {lijn: {home, away}}. Nul extra API-calls, geen pick-impact. (b) DATA-INTEGRITEITSVANGNET: nieuwe view v_clv_gaps (settled pick zonder clv_results-rij) wordt door /health gelezen -> warning settled_zonder_clv(n: ids) + veld clv_gaps. Dit gat was tot nu toe alleen met het blote oog te zien (clv_rows 17 vs picks_settled 18); met 19 competities schaalt dat niet. Zelfde vangnet-patroon als rls_gat (v206). Health-monitor alarmeert automatisch. // v219: CLV-GAT OP GOAL-MARKTEN GEDICHT. snapshotOddsOnly (de 23-05 UTC cron-gap-run) riep fetchOddsForFixtures aan met drie argumenten, dus enableGoals viel terug op de default false -> goal_odds bleef NULL in exact die snapshots die voor late WK-kickoffs (Amerika) de SLOTKOERS vormen. Gevolg: settled goal-markt-picks zonder CLV-rij (Mexico-England 06-07, O2.5 @2.38, 7 snapshots, 0x goal_odds). Nu enableGoals=ENABLE_GOAL_MARKETS + maxCalls 12->20 (de goal-loop self-limit't binnen maxCalls; geen AI in deze run). Daarnaast loggen gefaalde/lege /odds?fixture=-calls in de goal-loop niet langer stil weg via Promise.allSettled -> dit was de blinde vlek achter de wisselende dagdekking (0-63%). Geen model-, pick- of selectie-impact: alleen snapshot-vulling + observability. // v218: EDGE-CACHE GEREPAREERD. v199 heeft sinds de invoering NOOIT gecacht — gemeten via de X-Cache-Debug-header uit v217: elke respons gaf 'skip:errors', ook een geslaagde (errors=[], results=11). Twee gestapelde bugs: (1) API-Football zet `errors` bij succes op een LEGE ARRAY, en [] is truthy in JS, dus `!data.errors` was altijd false; (2) de payload-check eiste Array.isArray(response), maar teams/statistics geeft een object. Nu: _hasErrors() (niet-lege array/object = echte fout) + _hasPayload() (niet-lege array OF object). Gevolg: elke analyse deed tot dan toe 9 verse API-calls, wat reproduceerbaar 'Too many requests' uitlokte; raakte dat teams/statistics, dan viel poisson.valid om naar false en verdween het model uit 1X2 + doelpunten + Asian lines onder de misleidende tekst 'te weinig vormdata'. Fouten en lege responses blijven ongecacht. // v217: OBSERVABILITY op de edge-cache. Gemeten: de v199-cache slaat voor GEEN ENKEL endpoint aan (nooit een X-Cache-Policy-header, herhaalde calls blijven de API raken). De put zat in een lege try/catch, dus de fout was onzichtbaar. Nieuwe X-Cache-Debug-header rapporteert per request: hit / miss / put-ok / put-fail:<reden> / match-fail / skip:<reden>. Geen gedragswijziging. // v216: BUGFIX edge-cache — teams/statistics werd nooit gecacht. De cache-conditie eiste Array.isArray(data.response), maar API-Football geeft voor teams/statistics een OBJECT terug (fixtures/goals/...); voor fixtures/standings/predictions een array. Het endpoint stond dus wel in de `cacheable`-lijst maar sloeg er altijd langs. Gevolg: elke losse analyse deed 2 verse statistics-calls — juist de calls die het Poisson-model voeden. Gemeten: een normale analyse-burst (9 parallelle calls) lokt reproduceerbaar 'Too many requests' uit; raakt dat teams/statistics, dan gaat apiFetch in backoff (2s/4s), overschrijdt de wt-timeout van 5s, levert null, en valt poisson.valid om naar false -> model verdwijnt uit 1X2 + doelpunten + Asian lines. Nieuwe helper _hasPayload() accepteert zowel niet-lege arrays als niet-lege objecten. Fouten/lege responses worden nog steeds nooit gecacht. // v215: MARKT-ANKER op het doelpuntentotaal (optie A), DORMANT achter model_config.lambda_anchor_w (default 0 -> picks byte-identiek). De Poisson-lambda's zijn SoS-blind; wijkt het model-totaal af van het markt-impliciete totaal, dan tonen Under 1.5/2.5/3.5 EN BTTS-Nee allemaal dezelfde schijn-value (France-Morocco: model 2.10 vs markt 2.62 -> +10.6/+14.2/+10.6/+11.4pp, vier keer dezelfde scheve parameter, geen edge). solveMarketTotal lost het markt-impliciete totaal per O/U-lijn op met dezelfde Poisson+Dixon-Coles-kern; marketTotalGoals middelt gewogen over 1.5/2.5/3.5 (2.5 telt dubbel) zodat geen enkele lijn per constructie op 0pp wordt vastgezet. anchorLambdas trekt het TOTAAL naar de markt met gewicht lambda_anchor_w en laat de supremacie lh:la exact ongemoeid; deadband 0.30 goal (daaronder is het ruis). Toegepast in buildGoalCandidates (picks) + buildAhShadowRow (AH-shadow). Bij w=1.0 zakken de vier schijn-values naar -0.8/+0.6/-0.1/+2.0pp. Validatie via v_ai_tip_accuracy + v_goal_market_performance voor de vlag omhoog gaat. w=0 no-op geverifieerd. // v214: asianProbsW + buildAhShadowRow ankeren de AH-matrix op de gecorrigeerde 1X2 (ai.h/x/a) i.p.v. een losse markt-pull op rauwe Poisson — zelfde methode als frontend v26.249, dus worker en frontend geven identieke AH-kansen. raw_model_pct blijft de ongeankerde Poisson voor anker-evaluatie. // v213: zelfde Asian Handicap-tekenflip-fix als frontend v26.247 in parseGoalConsensus (bet 4) — away-lijn niet meer negeren (k=ahKey(raw)). Dit voedde de AH-shadow-validatie met verkeerd-gekoppelde odds -> kapotte de-vig; nu correct. Bestaande ah_shadow_picks-rijen (bug-tijdperk, dun/WK) zijn niet betrouwbaar maar AH stuurt geen picks, dus geen impact op productie. // v212: /health toont Elo-blend validatie-status — leest v_elo_blend_backtest + elo_blend_w, geeft elo_blend-object (n_duels, brier/roi/hitrate model vs blend, ready) en pusht waarschuwing 'elo_blend_valideren(n=X)' zodra n_duels>=ELO_VALIDATE_MIN_N(40) EN elo_blend_w=0 EN niet-gedismissd. Health-monitor pikt dit op = agenda-onafhankelijke herinnering. Dismiss via model_config.elo_blend_validated=1 of de vlag omhoog. Raakt picks NIET. // v211: Elo-blend SCHADUW-LOG voor validatie — bij rijpheid (beide teams >=6 games) logt de scan per duel de ongeblende AI-kans, de Elo-implied kans en de geblende kans (referentiegewicht ELO_SHADOW_W=0.30) + consensus-odds naar tabel elo_shadow (RLS+service_role). De sweep settelt die rijen met de uitslag. View v_elo_blend_backtest vergelijkt Brier (kanskwaliteit), favoriet-hitrate en value-ROI van model vs blend. Raakt de picks NIET (live-blend blijft gated op elo_blend_w=0). Zo is de okt-validatie 1 query. // v210: updateEloForFixture gebruikt eloHomeAdv(leagueId) (0 toernooi/WK-neutraal, 65 clubs) i.p.v. vast 40 — consistent met blend + markt-seeding. Geldt voorwaarts; de 14 WK-teams uit de eerste sweep houden hun 40-gebaseerde waarde (1 game elk, verwaarloosbaar, Elo corrigeert zichzelf). // v209: Elo-accumulatie losgekoppeld van schaduw-picks — nieuwe sweepEloFromResults werkt team_ratings/elo_history bij uit ALLE afgeronde wedstrijden in de actieve competities (FASE 1/2 via activeLeagueIds), dagelijks getthrottled via model_config.elo_last_sweep, venster gisteren+eergisteren, batch-dedup tegen elo_history + skipDup op updateEloForFixture, chronologisch verwerkt, gecapt op ELO_SWEEP_MAX(60) voor subrequestbudget. Aangeroepen in /settle (ontkoppeld try/catch). Raakt picks NIET (blend blijft dormant, elo_blend_w=0). Lost op dat de Elo voorheen alleen schaduw-pick-fixtures zag en dus leeg bleef. // v208: Elo/SoS-blend geinstalleerd (bouwplan stap 1-3+5), DORMANT achter model_config.elo_blend_w (default 0 -> picks byte-identiek). Team-ID plumbing in beide scan-paden; scan-helpers eloExpected/eloImplied1X2/marketToEloDiff/eloHomeAdv; markt-seeding van onbekende teams (ENABLE_ELO_FOUNDATION, rijpt de Elo, raakt picks niet, geen clobber want alleen bij geslaagde preload); blend op ai.h/x/a voor candidate-vorming, alleen actief als elo_blend_w>0 EN beide teams >=ELO_MIN_GAMES(6), begrensd op ELO_MAX_W(0.5). Validatie via v_ai_tip_accuracy na 20-07 voor de vlag omhoog gaat. w=0 no-op geverifieerd. // v207: RLS-vangnet compleet — v_rls_audit vangt nu OOK permissive policies op public/anon/authenticated met USING/CHECK(true) (het 'RLS-aan-maar-policy-open'-lek), niet alleen tabellen zonder RLS; /health meldt object/issue // v206: security-vangnet — /health leest v_rls_audit en meldt 'rls_gat(...)' zodra een publieke tabel zonder RLS opduikt; de dagelijkse health-monitor alarmeert daar automatisch op (ok=false). Voorkomt dat een RLS-gat weer maandenlang open kan staan // v205: Asian Handicap schaduw-trackrecord — AH-odds (bet 4) meegelezen in parseGoalConsensus (zelfde per-fixture call, nul extra API-load), asianProbsW (Poisson+DC, kwartlijnen gesplitst, push-conditioneel) + mismatch-anker; beste AH-lijn per wedstrijd naar ah_shadow_picks (GEEN invloed op picks), settleAhShadow met win/half_win/push/half_loss/lose op 90-minuten-uitslag (score.fulltime), /ah-shadow endpoint // v204: /model-tips staat gelijkspel toe als value-tip bij evenwichtige duels (geen dominante favoriet >=60%) — card sluit nu aan op de analyse die gelijkspel ook meeneemt (optie 1). Valkuil-draws in mismatches blijven geweerd // v203: CLV-slotkoers voor goal-markten (O/U + BTTS) uit goal_odds JSONB i.p.v. foute terugval op away_odds. Lost de absurd-negatieve CLV op O/U/BTTS op (bv. BTTS closing 12.25 -> nu de echte over/under/btts-slotkoers). 1X2 was al correct // v202: Elo-fundament LIVE — updateEloForFixture werkt team-ratings bij na elke afgeronde wedstrijd (gewogen naar tegenstander-Elo = SoS). Nu alleen accumuleren; modelintegratie na 20 juli. Team-ID-koppeling via het fixture-object in de settlement // v201: /model-tips filtert gelijkspel + extreme longshots eruit en geeft alleen positieve value — bij een klasse-mismatch (bv. Argentinie-Kaapverdie) geen nep-draw/longshot-tip meer op de card, valt netjes terug op MARKT-favoriet // v200: klasse/SoS-verankering in de scan-prompt (spiegelt v26.216 van de analyse) - marktodds als klasse-anker, geen model-kans die een duidelijke favoriet <65% zet op rauwe doelsaldi. Dicht de Poisson-valkuil (Argentinie 40% e.d.). Elo-tabel als fundament voor diepere SoS-fase // v199: edge-caching in de API-Football-proxy voor traag-veranderende data (vorm/stats/predictions/stand/h2h/blessures, 6u) — drastische besparing op de daglimiet bij herhaalde analyses; odds blijven vers // v198: /model-tips geeft nu de VALUE-pick (model - markt na de-vig) i.p.v. de model-favoriet — TIP-hoekje op de card komt zo overeen met de value-verankerde losse analyse (lost card-vs-analyse tegenspraak op) // v197: temperature:0 op ALLE scan-AI-calls (hoofdscan, test, daily-tip, nieuws) — cron-scan nu deterministisch zodat modelkansen/picks en het TIP-hoekje niet meer per run wisselen // v196: MAX_AI_ANALYSES_PER_SCAN 12->24 — meer vooruit scannen zodat verdere wedstrijden sneller een echte AI-tip krijgen i.p.v. de MARKT-plaatshouder // v195: auto-kalibratie op ACTIEF (AUTOTUNE_APPLY=true) — tuner past voortaan zelf bij, maar pas na 20-07 met >=40 clubtips/band; klein/begrensd/gelogd/omkeerbaar // v194: auto-kalibratie (optie 1) — bias-shrink runtime-instelbaar via model_config, wekelijkse tuner op clubdata (DRY-RUN: AUTOTUNE_APPLY=false), volledig gelogd in calibration_tune_log, endpoint /autotune // v193: /model-tips endpoint (model-favoriet 1X2 per aankomende wedstrijd) voor TIP-hoekje op value-loze cards // v192: goal-markt-tips (O/U + BTTS) ook getrackt op accuraatheid — modelrijen in model_market_comparison, settleModelTips rekent af via settleGoalMarket, view v_ai_tip_by_market + endpoint /ai-accuracy // v191: AI/model-tip-accuraatheids-tracking — settleModelTips rekent model-tips af (uitslag in model_market_comparison.won) zodat view v_ai_tip_accuracy continu kalibratie/hitrate/ROI per kans-band toont // v190: tier-gebaseerde bias-correctie op 1X2-value — extra market-shrinkage in het lage-kansgebied (aiKans<20% en 20-35%) waar de WK-backtest overschatting van longshots/draws aantoonde; conservatief + instelbaar (LOWPROB_EXTRA_SHRINK_1/2), hertunen op clubdata na 20-07 // v189: limieten opgehoogd na Workers Paid — app-analyses gedeelde key 5->25/gebruiker & 60->300 globaal (testfase), handmatige scan-cap 25->40, /health dagcap-warn 25->35. maxPerDay blijft ongebruikt (dode variabele, cron heeft geen dagcap) // v188: BUGFIX scanvenster — runScan scanTo stond op 18 UTC terwijl cron t/m 22 UTC vuurt en /health t/m 22 UTC monitort; avondscans (18-22 UTC = prime-time NL) werden stil overgeslagen. scanTo -> 23 zodat alle door de cron geplande volledige scans ook echt draaien // v187: scan-robuustheid voor 19-competitie-fase — AI-cap als tunable constante MAX_AI_ANALYSES_PER_SCAN (12), soonest-first sort laat fixtures zonder aftraptijd niet langer batch-slots afsnoepen, + deferral-log wanneer meer fixtures dan de cap in het venster zitten // v186: FASE 1 uitgebreid — WK + actieve zomercompetities (103 Eliteserien NO, 113 Allsvenskan SE) zodat de scanner tijdens de WK-tail doorbouwt aan het trackrecord; LEAGUE_FACTORS labels 113/103 rechtgezet (waren verwisseld, waarde ongewijzigd 0.88) // v185: LEAGUE_FACTORS rechtgetrokken + FASE 2 uitgebreid — 94 was fout 'Jupiler' gelabeld (is Portugal), 119 was 'Eredivisie playoffs' (is Denemarken); factoren toegevoegd voor 144/203/179/207/79/43/80/41 (stonden op default 0.92) // v184: FASE 2 league-fix — id 197 (Super League 1 Griekenland) was foutief gelabeld als Zwitserland; gecorrigeerd naar 207 (Super League Switzerland) conform bedoeling // v183: /health scan_verouderd-venster gelijkgetrokken met echte scan-uren (12-16 -> 12-22 UTC) — avond-aftrappen post-WK nu wel gemonitord // v182: dode prompt-caching opgeruimd in hoofdscan — system-blok (~30 tokens) lag onder de 1024-token cachedrempel en deed dus niets; beta-header + lege cache-markering verwijderd (geen kostenverschil) // v181: kosten van directe cron/systeem-Anthropic-calls (cron-scan, scan-test, daily-tip, oranje-nieuws) nu OOK in user_costs via trackAnthropicCost — in-app totaal benadert nu de echte Anthropic-rekening // v180: kosten-registratie gerepareerd — schreef naar niet-bestaande kolommen (month/ai_calls/total_cost); nu atomair ophogen via RPC increment_user_cost (calls/tokens_in/tokens_out/total_usd) // v179: gedeelde-key daglimiet verlaagd voor pre-launch — 5/gebruiker, 60 globaal (was 50/400); raakt alleen app-analyses, niet de cron-scans. Eigen-key-gebruikers onbeperkt // v178: eigen Anthropic-key support — frontend stuurt x-user-anthropic-key mee; worker gebruikt die i.p.v. env-key, slaat daglimiet + kostentracking over (gebruiker betaalt zelf) // v177: /health activeHours teruggebracht naar werkelijke scan-uren (06 + 12-16 UTC) — geen valse scan_verouderd-WARN meer 's avonds/'s nachts // v176: /goal-markets endpoint (serveert v_goal_market_performance incl. CLV) voor de app // v175: closing O/U + BTTS-odds meegeschreven in odds_snapshots (goal_odds jsonb) voor CLV op doelpunten-markten — zonder extra API-calls (hoofdscan haalt ze al op) // v174: doelpunten-markten LIVE in productie-cron (O/U 1.5/2.5/3.5 + BTTS als volwaardige picks, vlag ENABLE_GOAL_MARKETS) + afrekening voor alle goal-markten; consistency-check per marktgroep, longshot-guard alleen op 1X2 // v173: goal-markten in scan-test (?goals=1) — Poisson uit AI-goals, 2-weg de-vig // v172: schaduw-vangnet — near-misses loggen op RUWE divergentie (>=3pp) onder de value<3-poort, voor volledigere draw-evaluatie (selectie ongewijzigd) // v171: schaduw-afrekening terug naar losse fixtures-calls (bewezen) i.p.v. gebatchte ?ids // v170: cron-stappen ontkoppeld (try/catch) zodat schaduw-afrekening altijd draait // v169: schaduw-afrekening robuuster — gebatchte fixtures-call + ook in cron-gap-uren // v168: schaduw-picks 1 per wedstrijd (sterkste bijna-misser) // v167: /shadow endpoint (schaduw-trackrecord voor app) // v166b: + settleShadowPicks (schaduw-picks afrekenen met uitslag) // v166: schaduw-trackrecord — bijna-value picks (longshot/draw/below_threshold) gelogd in shadow_picks // v165: aftraptijd (match_time) opgeslagen in model_market_comparison + doorgegeven aan sharp-data (verberg al-gespeelde wedstrijden) // v164: verouderd Sonnet 4 model vervangen door claude-sonnet-4-6 (daily tip + oranje nieuws) // v163: /health endpoint (versie, laatste scan, picks, CLV, snapshot-dichtheid + warnings) // v162: scans_today reset op nieuwe dag in hoofdpad (teller liep eindeloos op, blokkeerde /scan-now) // v161: filter licht versoepeld — shrink 0.45/0.55, draw-straf 0.88/0.90, draw-minValue lager, strong-draw guardrail-uitzondering // v160: /scan-now totaal-dagcap 25 (begrenst handmatige scan-kosten) // v159: /scan-now endpoint (handmatige scan vanuit app, cooldown 60s + daglimiet) // v158: handmatig scanpad — ondergrens aftraptijd (geen al-gespeelde wedstrijden) // v157: value-hardening — model-shrinkage naar markt (0.50 / toernooi 0.65) + favorite-longshot guardrail (odds>=3.5 vereist sharpScore>=55) // v156: snapshot-only cron-run 23-05 UTC voor late WK-kickoffs (verse slotkoers) // v155: CLV-fix — snapshot ALLE aankomende fixtures (opening->closing curve) + saveCLV valt terug op snapshot-slotkoers + niet meer bailen op lege live-CLV // v154: sharp-tier drempels in constanten (SHARP_TIERS) // v153: WK-only scan tijdens WK-zomer (FASE 1 = alleen league 1) // v152: cache-bust op odds fetch // v151: drempels terug naar productie // v151-TEST: drempels verlaagd voor test — TIJDELIJK // v150: steam 6%, sharp score ≥55, geen gelijkspel, geen gespeeld // v149: post-WK leagues — KKD + 2/3.Bundesliga + Championship + League One // v148: automatische seizoenswisseling — WK-zomer → Europees seizoen (20 jul) // v147: 24→11 actieve leagues + bulk odds fetch // v146: bulk datum odds fetch — 2 calls i.p.v. 24+ (rate limit fix) // v145: league tiers + pick tier performance + Monte Carlo // v144: AI invloed teruggebracht naar 10% — markt (fairImplied) domineert 40% // v143: prompt caching ingeschakeld — ~70% token besparing op scans // v142: scan analyses via Sonnet 4.6 ipv Haiku (betere kwaliteit) // v141: pick consistency lock + gelijkspel 2-scan bevestiging // v140: poissonMap doorgegeven aan detectSharpMoney — divergentie nu correct // v139: betere WK AI-prompt (FIFA/form), push timing 6u voor aftrap // v138: WK_ONLY_MODE uit + alle actieve leagues + WK drempel conf5/value6 + elite ook WK // v137: 1 pick per wedstrijd + strengere drempels (minValue 3→6, minConf 5→6) // v136: rate limits 15→50 user, 150→400 globaal // v135: elite sharp money engine — market_consensus + model_market_comparison + sharp_signal_results // v134: geen push bij lege scan // v133: scan-test default league 1 (WK)
...
async function fetchOddsForFixtures(fixtureIds, env, maxCalls = 36, enableGoals = false, matches = null, stats = null) {
...
async function runScan(env, force = false) {
  ...
  const oddsMap = await fetchOddsForFixtures(fixtureIds, env, 80, ENABLE_GOAL_MARKETS, allMatches);
  ...
  async scheduled(event, env, ctx) {
    ...
    ctx.waitUntil((async () => {
      ...
      if (fullScan) {
        ...
        // throttlet zichzelf op 1x/dag via model_config.elo_last_sweep, dus de overige 11 scans van de
