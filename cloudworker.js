    ...
    if (path === '/health') {
      try {
        ...
        return json({ ok: warnings.length === 0, status: warnings.length ? 'WARN' : 'OK',
          warnings, worker_version: VERSION, ...h, elo_blend: eloBlend, checked_at: new Date().toISOString() });
      } catch(e) {
