    ...
    if (path === '/health') {
      try {
        ...
        try {
          ...
          if (Array.isArray(gr)) {
            goalOddsStatus = {};
            let onsProbleem = 0, totaal = 0;
            for (const r of gr) {
              const n = Number(r.snapshots);
              if (!Number.isFinite(n)) continue;
              goalOddsStatus[r.status] = n;
              totaal += n;
              if (r.status === 'gefaald' || r.status === 'rate_limited' || r.status === 'geen_budget') onsProbleem += n;
            }
            ...
            if (totaal >= GOAL_ODDS_MIN_N && onsProbleem > 0) {
              const pct = Math.round((onsProbleem / totaal) * 1000) / 10;
              if (pct >= 5) warnings.push(`goal_odds_eigen_verlies(${pct}% van ${totaal})`);
            }
          }
        } catch(e) { /* health niet laten falen op een diagnoseveld */ }
