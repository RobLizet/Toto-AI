...
export default {
  async fetch(request, env) {
    ...
    if (path === '/vvv-news') {
      ...
      // Bron 3 -- Google News RSS. Levert regionale media die de andere twee niet hebben
      // (L1 Nieuws, Omroep Venlo, De Limburger, ESPN). Gemeten op de live feed: 0 items van
      // Voetbalprimeur/Voetbalzone, dus die dekt Google News NIET af. Items van VVV-Venlo en
      // Venlonaren worden hier overgeslagen: die halen we hierboven rechtstreeks op, met een
      // echte artikel-link i.p.v. een news.google.com-omleiding.
      // LET OP: v233 haalde Google News eruit wegens 503 vanuit Cloudflare-IP's. Deze bron staat
      // daarom in dezelfde allSettled -- faalt hij, dan meldt `sources` dat en blijven de andere
      // twee gewoon staan. De 503 is vanuit een Worker niet vooraf te meten, alleen zo.
      const bronGoogleNews = async () => {
        const url = 'https://news.google.com/rss/search?q=%22VVV-Venlo%22&hl=nl&gl=NL&ceid=NL:nl';
        ...
        const eigen = ['vvv-venlo', 'venlonaren'];
        const out = [];
        for (const raw of xml.split('<item>').slice(1)) {
          const b = raw.split('</item>')[0];
          let title = tag(b, 'title');
          const link = tag(b, 'link');
          const bron = tag(b, 'source');
          if (!title || !link || !bron) continue;
          if (eigen.includes(bron.toLowerCase())) continue;         // hebben we al, met betere link
          // Google zet de bron als " - Naam" achter de titel; die staat al in het bron-label.
          if (title.endsWith(' - ' + bron)) title = title.slice(0, -(bron.length + 3)).trim();
          if (!/vvv|venlo/i.test(title)) continue;                  // query matcht ook de bodytekst
          let pubDate = '';
          const d = new Date(tag(b, 'pubDate'));
          if (!isNaN(d.getTime())) pubDate = d.toISOString();
          out.push({ title, link, pubDate, source: bron });
          if (out.length >= 12) break;
        }
        return out;
      };

      const res = await Promise.allSettled([bronOfficieel(), bronVenlonaren(), bronGoogleNews()]);
      const namen = ['VVV-Venlo', 'Venlonaren', 'GoogleNews'];
      const bronnen = {};
      let items = [];
      res.forEach((r, i) => {
    ...
    if (path === '/health') {
      try {
        ...
        return json({ ok: warnings.length === 0, status: warnings.length ? 'WARN' : 'OK',
          warnings, worker_version: VERSION, ...h, clv_gaps: clvGaps, elo_blend: eloBlend, checked_at: new Date().toISOString() });
      } catch(e) {
