...
function extractStandingInfoWorker(standings, teamId, leagueId) {
  ...
  if (isKKD) {
    ...
    else if (pos <= 8) motivatieLabel = `promotie-play-offrace (${pos}e)`;
...
${analyseBatch.map((m, i) => {
  ...
  if (st) {
    ...
    if (standTable) {
      const hStand = extractStandingInfoWorker(standTable, m.homeId, m.leagueId);
      const aStand = extractStandingInfoWorker(standTable, m.awayId, m.leagueId);
