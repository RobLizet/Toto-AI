...
async function apifProxyStatus(env) {
  ...
  try {
    ...
    if (rijen === null) {
      ...
      for (const r of rijen) {
        const c = Number(r.api_calls);
        if (!Number.isFinite(c)) continue;
        calls += c;
        if (Number.isFinite(Number(r.api_refused))) {
          const w = Number(r.api_refused);
          pogingen += w;
          if (c > 0) { const pct = (w / c) * 100; if (piek === null || pct > piek) piek = pct; }
        }
        if (Number.isFinite(Number(r.api_errors))) fouten += Number(r.api_errors);
        // api_opgegeven bestaat pas vanaf v297; oudere rijen zijn NULL en tellen niet mee.
        if (Number.isFinite(Number(r.api_opgegeven))) { opgegeven += Number(r.api_opgegeven); opgegevenGemeten++; }
      }
      uit.trend_24u = {
        gemeten: true,
        scans: rijen.length,
