  ...
  async fetch(request, env) {
    ...
    if (path === '/health') {
      try {
        ...
        try {
          const rlsGaps = await sb(env, 'v_rls_audit', 'GET', null, '?select=table_name') || [];
          if (rlsGaps.length) warnings.push('rls_gat(' + rlsGaps.map(r => r.table_name).join(',') + ')');
