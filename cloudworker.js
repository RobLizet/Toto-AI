...
export default {
  async fetch(request, env) {
    ...
    if (path === '/vvv-news') {
      try {
        const bron = 'https://www.vvv-venlo.nl/nieuws';
        ...
        while ((m = re.exec(html)) !== null && items.length < 20) {
          ...
          // Datum DD-MM-YYYY aan het eind → ISO YYYY-MM-DD; titel = de rest.
          let pubDate = '';
          ...
          if (!txt || txt.length < 9) continue;
          seen.add(slug);
          items.push({ title: txt, link: 'https://www.vvv-venlo.nl' + slug, pubDate, source: 'VVV-Venlo' });
        }
