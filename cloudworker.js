    ...
    if (path === '/health') {
      try {
        ...
        return json({ ok: warnings.length === 0, status: warnings.length ? 'WARN' : 'OK',
          warnings, versie_info: versieInfo, worker_version: VERSION, ...h, odds_dekking: oddsDek, supabase_sleutel: sleutelVorm, push: pushStat, clv_gaps: clvGaps, clv_per_markt: clvMarkt, elo_blend: eloBlend, apif_proxy: apifStatus, checked_at: new Date().toISOString() });
      } catch(e) {
