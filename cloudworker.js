...
async function apif(path, env) {
  ...
  for (const host of hosts) {
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        if (!out.length && _rateLimited) out.rateLimited = true; // v252: zie boven
        return out;
      } catch(e) { break; }
    }
  }
  ...
  const leeg = [];
  if (_rateLimited) { leeg.rateLimited = true; console.warn(`[apif] rate-limited na retries: ${path.slice(0, 60)}`); }
  return leeg;
}
...
async function runScan(env, force = false) {
  ...
  try {
    ...
    if (schedule) {
      ...
      console.log('[Scan] scan_schedule geladen, scanvenster 06:00-22:00 UTC (gelijk aan cron)');
    }
  } catch(e) {
