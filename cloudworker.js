...
async function apif(env, endpoint) {
  try {
    const r = await fetch(`https://v3.football.api-sports.io${endpoint}`, {
      headers: { 'x-rapidapi-key': env.FOOTBALL_KEY, 'x-rapidapi-host': 'v3.football.api-sports.io' },
...
// Haal odds op voor een fixture — probeert meerdere bookmakers
// Bookmaker IDs: 8=Bet365, 1=10Bet, 4=Bwin, 6=Betsafe, 11=1xBet, 16=Betway
const BOOKMAKER_PRIORITY = [8, 1, 4, 6, 11, 16];

async function fetchOdds(env, fixtureId) {
  // Eerst: probeer met specifieke bookmakers (sneller)
  for (const bmId of BOOKMAKER_PRIORITY) {
    const data = await apif(env, `/odds?fixture=${fixtureId}&bookmaker=${bmId}`);
    const bookmakers = data?.response?.[0]?.bookmakers || [];
    for (const bm of bookmakers) {
      const match = bm.bets?.find(b => b.name === 'Match Winner');
      if (!match) continue;
      ...
      if (o1 > 1 && oX > 1 && o2 > 1) {
        console.log(`[Odds] ${fixtureId} via bookmaker ${bm.name || bmId}`);
        return { home: o1, draw: oX, away: o2, bookmaker: bm.name };
      }
    }
  }
  // Fallback: haal alle bookmakers op en pak de eerste met geldige quotes
  const data = await apif(env, `/odds?fixture=${fixtureId}`);
  const bookmakers = data?.response?.[0]?.bookmakers || [];
  for (const bm of bookmakers) {
    const match = bm.bets?.find(b => b.name === 'Match Winner');
    if (!match) continue;
    ...
    const o2 = parseFloat(match.values.find(v => v.value === 'Away')?.odd);
    if (o1 > 1 && oX > 1 && o2 > 1) {
      console.log(`[Odds] ${fixtureId} fallback via ${bm.name}`);
      return { home: o1, draw: oX, away: o2, bookmaker: bm.name };
    }
  }
  return null;
