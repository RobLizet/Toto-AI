...
const LOWPROB_EXTRA_SHRINK_1 = 0.15; // aiKans < 20%
const LOWPROB_EXTRA_SHRINK_2 = 0.08; // aiKans 20–35%
...
function calculateValue(aiKans, fairImpliedPct, pick, marketShrink = 0) {
  ...
  if (pick === '1' || pick === 'X' || pick === '2') {
    if (aiKans < 20)      extra = LOWPROB_EXTRA_SHRINK_1;
    else if (aiKans < 35) extra = LOWPROB_EXTRA_SHRINK_2;
  }
...
const MAX_AI_ANALYSES_PER_SCAN = 12; // v187: max AI-analyses per run (soonest-first). Tunable voor de 19-competitie-fase; begrenst subrequests/kosten. Rest schuift door naar de volgende uur-run.
...
export default {
  async fetch(request, env) {
    ...
    if (path === '/model-tips') {
      // v193: model-favoriet (hoogste Poisson-kans, 1X2) per aankomende wedstrijd — voor het TIP-hoekje op cards zonder value
      try {
        const today = new Date().toISOString().split('T')[0];
        const rows = await sb(env, 'model_market_comparison', 'GET', null,
          `?match_date=gte.${today}&won=is.null&pick=in.(1,X,2)&select=fixture_id,pick,poisson_win_pct&order=fixture_id,poisson_win_pct.desc&limit=3000`) || [];
        const best = {};
        for (const r of rows) {
          const fid = String(r.fixture_id);
          if (!best[fid]) best[fid] = { fixture_id: r.fixture_id, pick: r.pick, pct: r.poisson_win_pct };
        }
        return json({ ok: true, tips: Object.values(best), worker_version: VERSION });
      } catch(e) {
        return json({ ok: false, error: e.message, worker_version: VERSION }, 500);
      }
    }
