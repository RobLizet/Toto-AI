...
async function fetchOddsForFixtures(fixtureIds, env, maxCalls = 36, enableGoals = false) {
  ...
  try {
    ...
    // v242: DOORPAGINEREN. Hier stonden pagina 1 en 2, hardgecodeerd, onder het commentaar dat de
    // datum-bulk 'alle odds voor die dag in 1 call' ophaalt. Dat klopt niet: /odds?date= levert 10
    // resultaten per pagina en paging.total was op 15-07 ACHT pagina's (16-07: zes). We zagen dus
    // ~20 van de ~80 fixtures met odds, plus de fallback van 5. Tijdens het WK viel dat niet op met
    // een handvol duels per dag; met 19 competities vanaf 20-07 zou het merendeel van de wedstrijden
    // stil zonder odds vallen -- en zonder odds komt een wedstrijd niet eens in analyseBatch, dus
    // geen analyse en geen picks. Gemeten op de droogloop: 1 van 24 fixtures gedekt, bij twee totaal
    // verschillende competities (CL/EL/ECL-kwalificatie en clubvriendschappelijke duels).
    // Nu doorlopen tot de data op is, met drie stopcondities: lege pagina, een pagina korter dan de
    // paginagrootte (= laatste), of alle fixtures gedekt. maxCalls blijft de harde begrenzing.
    const ODDS_PAGE_SIZE = 10;   // API-Football levert 10 odds-items per pagina
    const ODDS_MAX_PAGES = 12;   // ruim boven de 8 gemeten pagina's; maxCalls kapt eerder af
    for (const date of [today, tomorrow]) {
      ...
      try {
        for (let page = 1; page <= ODDS_MAX_PAGES; page++) {
          ...
          if (r.length < ODDS_PAGE_SIZE) break;                      // laatste pagina
          if (fixtureIds.every(id => oddsMap[id])) break;            // alles al gedekt
        }
      } catch(e) { console.error(`[Odds] Bulk datum ${date} fout:`, e.message); }
    }
    console.log(`[Odds] bulk klaar: ${fixtureIds.filter(id => oddsMap[id]).length}/${fixtureIds.length} gedekt na ${oddsCallsUsed} calls`);
    ...
    if (path === '/health') {
      try {
        ...
        return json({ ok: warnings.length === 0, status: warnings.length ? 'WARN' : 'OK',
          warnings, versie_info: versieInfo, worker_version: VERSION, ...h, clv_gaps: clvGaps, elo_blend: eloBlend, checked_at: new Date().toISOString() });
      } catch(e) {
