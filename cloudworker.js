...
const FASE2_LEAGUES = [
  ...
  // v303: Scandinavische zomercompetities (seizoen apr-nov, spelen nu)
  { id: 103, s: 2026 }, // Eliteserien (Noorwegen)
  { id: 113, s: 2026 }, // Allsvenskan (Zweden)
];

function activeLeagueIds(today) {
  // v304: leest dezelfde constante als runScan -- geen tweede lijst meer om te vergeten.
  const postWK = new Date(today) >= new Date('2026-07-20');
  return postWK ? FASE2_LEAGUES.map(l => l.id) : [1, 103, 113];
}
  ...
  async fetch(request, env) {
    ...
    if (path === '/leagues') {
      return new Response(JSON.stringify({
        ...
        gescand: FASE2_LEAGUES.map(l => ({ id: l.id, seizoen: l.s })),
      }), {
        headers: {
          'Content-Type': 'application/json',
