...
function startAutoCheckScheduler() {
  ...
  async function runScheduledScan() {
    ...
    if (state.settings.scanSkipDate === now.toDateString()) return;
    if (!state.settings.autoScan) return;

    showAutoCheckBar('⏱ Auto scan gestart...');

    try {
      // Laad vandaag alle competities als nog geen matches
      if (!state.matches || !state.matches.length) {
        if (typeof loadTodayAllComps === 'function') {
          await loadTodayAllComps();
          await new Promise(r => setTimeout(r, 2000)); // wacht op laden
        }
      }
      ...
      if (!candidates.length) {
        showAutoCheckBar('⏱ Geen wedstrijden gevonden', 3000);
        return;
      }

      showAutoCheckBar(`⏱ Scannen ${candidates.length} wedstrijden...`);

      // Gebruik scanValueAll als die beschikbaar is
      if (typeof scanValueAll === 'function') {
        await scanValueAll();
        // scanValueAll slaat op in state.valueScans
        const picks = (state.valueScans || [])
          .filter(v => (v.value||0) >= (state.settings.notifThreshold||15))
          .map(v => ({ ...v, scannedAt: now.toISOString(), source: 'auto' }));

        if (picks.length) {
          // Voeg toe aan scheduledScanPicks, voorkom duplicaten
          const existing = state.scheduledScanPicks || [];
          const newIds = new Set(picks.map(p => p.id));
          const filtered = existing.filter(p => !newIds.has(p.id));
          state.scheduledScanPicks = [...picks, ...filtered].slice(0, 50);
          saveState();

          showAutoCheckBar(`✅ ${picks.length} value pick${picks.length>1?'s':''} gevonden!`, 5000);

          // Push notificatie als ingeschakeld
          if (state.settings.notifEnabled && state.settings.autoValueAlerts) {
            const top = picks[0];
            sendValueNotification(
              `⚡ ${picks.length} value pick${picks.length>1?'s':''} gevonden`,
              `Beste: ${top.home} vs ${top.away} — ${top.value?.toFixed(1)}% value`,
              top.id, top.value, null
            );
          }
        } else {
          showAutoCheckBar('✓ Geen value picks boven drempel', 3000);
        }
      }
    } catch(e) {
      console.error('[AutoScan] Fout:', e);
      showAutoCheckBar('⚠ Auto scan mislukt', 3000);
    }
  }

  // Direct één keer draaien
  runScheduledScan();

  // Dan elk uur herhalen
  autoCheckInterval = setInterval(runScheduledScan, 60 * 60 * 1000);
}
